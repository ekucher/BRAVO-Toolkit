...
try {
    ...
    if ($null -ne $global:bravoDiscoveryResult) {
        ...
        # Presence-контракт (#158, етап 2): явний стан КОЖНОГО компонента
        # окремо від сирого шляху. Без цього виводу Ambiguous і Error
        # виглядали для оператора однаково — як порожнє значення поля,
        # тобто як звичайна відсутність компонента. Саме форматування
        # належить BRAVO.Discovery (власник контракту), тут лише виклик.
        Write-BRAVODiscoveryPresenceReport -DiscoveryResult $bravoDiscoveryResult
        ...
        #
        # Root cause реальної VM-аномалії "The property 'Count' cannot be
        # found" (2026-09-18, підтверджено з точним стек-трейсом і
        # відтворено локально): `$var = if (cond) {...} else {...}`,
        # використаний як value-providing вираз, ЗАВЖДИ ризикує
        # розгорнути 0-елементний масив у $null через власне unwrapping
        # if-блоку у вихідний потік — саме `@(...)`-обгортка ВСЕРЕДИНІ
        # кожної гілки цього НЕ рятує (перевірено емпірично: @(Test-Foo),
        # де Test-Foo повертає @(), усе одно колапсує в $null після
        # проходження через if-як-вираз). Унарний кома-оператор `,`
        # застосований ЗОВНІ if-блоку через `$(...)` теж НЕ рятує — він
        # загортає весь результат if-виразу як ОДИН елемент (Count завжди
        # 1, без сплощення). Правильний фікс — кома ВСЕРЕДИНІ кожної
        # гілки, безпосередньо перед `@(...)`: тоді PowerShell передає
        # вже готовий масив як єдиний pipeline-об'єкт, і зовнішнє
        # присвоєння його не розгортає.
