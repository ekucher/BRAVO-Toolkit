...
- **Повний `BRAVO_SELF_TEST.ps1` став дешевшим без втрати покриття
  ...
  **Що саме було вузьким місцем** (за таймінгами самого self-test, не за
  здогадкою): чотири статичні аналізи — порожній `catch` без пояснення,
  statement-keyword у позиції команди, half-formatted `-f`-конкатенація
  і credential-shaped рядкові літерали — кожен окремо відкривав і
  ПОВНІСТЮ розбирав КОЖЕН файл комплекту. Це чотири повні проходи
  `Parser::ParseFile` по ~4.7 МБ коду. Тепер розбір спільний: один AST на
  файл живить усі чотири аналізи. Фільтри, тексти й ПОРЯДОК assert-ів не
  змінені.
  ...
  видаленням перевірки не вийде) і що цикл синтаксичної перевірки
  `Parser/<file>` лишається окремим, а також
  `OwnSourceMemoSurvivesAlternatingAccess` — регресія на наївне memo, де
  кожен getter скидає сусідній.
