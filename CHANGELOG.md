...
## 5.2.0-rc.2 — 2026-08-23 (candidate, pending acceptance)

Відкрито одразу після реального DEV-LIMS acceptance-прогону `rc.1`
(ДНДІЛДВСЕ, 2026-08-23), який виявив три проблеми — одна з них P0
data-integrity. Це функціональні runtime-зміни, тому acceptance-докази
`rc.1` більше не покривають runtime: **rc.2 потребує нового повного
acceptance-прогону** перед будь-якою stable promotion.

- **FIX (P0, data-integrity):** false-positive rollback моделі MODEL
  після `bravocmd repair` (нижче) — знайдено й підтверджено byte-точним
  збігом розмірів файлів на живому DEV-LIMS під час acceptance `rc.1`.
- **FIX (P0, data-integrity, друга й окрема причина):** `Compare-FileSizes`
  досі читала MODEL через ненадійний `Get-ChildItem -Recurse` (нижче) —
  на живому DEV-LIMS (ДНДІЛДВСЕ, 2026-08-24) це детерміновано (4/4 рази,
  незалежно від settle-retry вікна) хибно позначало майже всю модель як
  втрачену, навіть одразу після підтвердженого цілісним rollback.
- UX/DIAGNOSTICS: `Import-BravoConfiguration` тепер збагачує повідомлення
  про помилку виконання `BRAVO.config` висновком `Get-BRAVOOSSupportTier`
  (нижче), коли причина — непідтримуване середовище (напр. PowerShell
  <4.0), замість голої `.NET NullReferenceException` без жодного натяку.
...
- Health-alert дедуп: типовий `RepeatAlertAfterHours` змінено `6` → `0`
  ...
  acceptance-прогону.
- Логування очікування операційного lock (нижче) — виявлена прогалина
  спостережності під час розслідування того самого інциденту.
- UX: живий прогрес-індикатор фази "Реставрація моделі" тепер показує
  під-крок (нижче) — раніше під час усього тривалого блоку (архівація
  перед реставрацією, `bravocmd`, settle-retry) консольний
  progress bar лишався статичним, ніби завис.

Детальні рішення — `RELEASE_POLICY.md` §20.

- FIX (data-integrity): `Compare-FileSizes` (`BRAVO_MAINTENANCE`,
  MODEL-реставрація) двічі підтверджено видавала **false-positive**
  "критична втрата даних" одразу після `bravocmd repair` і одразу після
  відкату з `before`-архіву — сканування каталогу MODEL відбувалось за
  ~1 сек. після завершення процесу, до того як великі (сотні МБ — 1+ ГБ)
  щойно записані `.md`-файли ставали видимими для directory-enumeration.
  Наслідок на реальному DEV-LIMS (2026-08-23): служба BRAVO зупинялась
  fail-closed (коректно, за дизайном), але через артефакт перевірки, а
  не реальну втрату даних — підтверджено байт-точним збігом розмірів
  файлів на диску одразу після хибного "ФАЙЛ ВІДСУТНІЙ" у лозі. Без
  фіксу це повторювалося б щоночі на плановому `Restore.Time`.
  `Compare-FileSizes` тепер повторює сканування+порівняння до
  `MaxSettleAttempts` разів із паузою `SettleDelaySeconds` (обидва
  параметризовані) — лише коли перший прохід знайшов критичні
  розбіжності; чисте сканування завершується без затримки. Fail-closed
  не послаблено: якщо файл дійсно відсутній після всіх спроб — критичний
  статус і відкат лишаються без змін.
  **Уточнення дефолтів (той самий день, повторний реальний інцидент):**
  початковий дефолт `3×5с=15с` виявився замалим — коренева причина не
  проста NTFS-затримка видимості, а **активний антивірус з real-time
  захистом** (`RealTimeProtectionEnabled=True`, без винятку для
  `LIMS\Model`/`ARCHIV`), який тримає щойно записані гігабайтні
  `.md`-файли залоченими; `Get-BRAVOFiles`/`Get-ChildItem -ErrorAction
  SilentlyContinue` тихо пропускає залоченого кандидата. Дефолт
  збільшено до `12×15с=180с` (defense-in-depth, обмежений бюджет,
  незначний коштом на тлі `ExecutionTimeLimitHours=18` у
  `schedulerSettings.Maintenance`). **Канонічний фікс — виняток
  антивірусу** (`Add-MpPreference -ExclusionPath "<LIMSRoot>\Model"`,
  задокументовано в `OPERATIONS.md`, розділ коду `43`); збільшене
  retry-вікно — лише другий рубіж захисту, не заміна винятку.
- **FIX (P0, data-integrity): друга й окрема причина того самого
  симптому — ненадійна enumeration, не AV.** Реальний повторний
  інцидент на ДНДІЛДВСЕ (2026-08-24, найбільша/найглибша модель серед
  усіх acceptance-майданчиків: `KZPpatArc2.md` 1.19 ГБ, глибока
  кирилична структура `#\Form\...`) показав, що навіть 180с
  settle-retry-вікна (вище) не рятує — рівно той самий набір ~364
  файлів систематично позначався "ФАЙЛ ВІДСУТНІЙ" усі 4 рази поспіль,
  включно з перевіркою одразу після rollback із щойно підтвердженого
  цілісним 7-Zip-архіву. Це виключає AV/timing-race (та давав би
  плаваючий, зменшуваний з часом результат, а не детермінований).
  Корінна причина: `Compare-FileSizes` досі сканувала MODEL через
  `Get-BRAVOFiles`/`Get-ChildItem -Recurse` (PowerShell-провайдер) — той
  самий ненадійний на великих/глибоких деревах каталогів механізм, від
  якого `Check-MdFileSizes` і before-snapshot у тому самому restore-flow
  вже свідомо відмовились на користь прямого `[IO.DirectoryInfo]::`
  `EnumerateFiles`. З `-ErrorAction SilentlyContinue` обрив enumeration
  мовчки повертав неповний список; повторний виклик тієї самої функції
  (settle-retry) давав ту саму помилку щоразу — причина не в затримці
  появи файлів, а в самій enumeration. Дані фізично ніколи не
  втрачались. Виправлено: `Compare-FileSizes` тепер використовує той
  самий надійний `[IO.DirectoryInfo]::EnumerateFiles` +
  Hidden/System-фільтр, що вже канонічні для двох інших enumeration-
  точок цього restore-flow. Settle-retry лишається (валідний захист від
  справжніх AV-рейсів на інших майданчиках); fail-closed поведінка не
  послаблена.
...
- **UX: живий прогрес під час "Реставрація моделі".** Той самий
  ...
  7-Zip, `bravocmd.exe repair` — кілька хвилин, перевірка результату,
  settle-retry до 180с) — усе це відображалось на консольному
  progress bar (`Write-Progress`) єдиним статичним написом "Реставрація
  моделі" без жодного руху, що при інтерактивному запуску виглядає як
  зависання. Додано `Write-BRAVOProgressDetail` перед кожним великим
  під-кроком і на кожній ітерації settle-retry (`Compare-FileSizes`) —
  та сама смуга прогресу тепер дописує живий деталь ("архівація моделі
  перед реставрацією...", "виконання bravocmd.exe repair...", "очікую
  появи файлів (спроба X з N)..."). Суто UX-зміна: не впливає на
  journal/лог (`Write-Log` виклики не змінено), exit-коди чи
...
## 5.2.0-rc.1 — 2026-08-23 (candidate, superseded by rc.2)
