...
Stable promotion від прийнятого `5.2.3-rc.3` (нижче).

> **ЗАПОВНИТИ ПЕРЕД MERGE У `master`:** сюди вписується фактичний результат
> real-server acceptance rc.3 (`RELEASE_POLICY.md` §9) — сервер(и), дата,
> перелік перевірених сценаріїв і посилання на evidence-документ. Станом на
> підготовку цього коміту acceptance rc.3 на `LIMS-TOP` ще не завершено
> (продовжується з місця зупинки rc.2). Без записаного PASS цей коміт
> НЕ мерджиться: `RELEASE_POLICY.md` §4 і §12.1 вимагають перевіреного RC.

Release-metadata-only зміни відносно прийнятого rc.3: `VERSION.json`
(`packageVersion` `5.2.3-rc.3` → `5.2.3`, `releaseChannel` `prerelease` →
`stable`, `releaseDate`), заголовки README.md/BRAVO_SETUP.md, цей розділ
CHANGELOG, `RUNTIME_MANIFEST.json` перегенеровано stamp-кроком
(`ci\Update-BRAVOVersionStamp.ps1 -Apply` → `ci\Update-BRAVORuntimeManifest.ps1
-Apply`). Жодних функціональних runtime-змін відносно прийнятого rc.3.

`ModuleVersion` в усіх `modules\*\*.psd1` уже дорівнює `5.2.3` з лінії rc —
при промоції не змінювався.
...
повторені, бо це єдиний розділ, який читає оператор при оновленні парку:
...
- **`ExcludedDrives` більше не приховує нестачу місця на required volume.**
  ...
  блокування залишається (`ExclusionIgnoredForRequiredVolume`). Якщо диск
  додавали в `ExcludedDrives` саме для обходу такого блокування — перевірте
  конфігурацію до оновлення.
