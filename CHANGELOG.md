...
- **Порівняння знімків ефективної конфігурації полагоджено — три дефекти,
  ...
  **Видалення `BRAVO.config` могло мовчки не статись.** У runbook крок 6
  викликав `Remove-Item` без `-ErrorAction Stop`: відмова за правами чи
  блокуванням файла нетермінальна, тож знімок C знявся б при живому
  primary-шарі, граф збігся б з A і порівняння прийняло б міграцію, яка
  нічого не прибрала. Додано `-ErrorAction Stop` і перевірку `Test-Path`
  перед знімком.

  **Відкат у runbook теж був неповним.** Він відновлював лише
  `BRAVO.config`, лишаючи додані значення в site-файлі — а той має вищий
  пріоритет, тож сервер виглядав би відкоченим, не будучи ним. Тепер
  site-файл резервується **до** зміни і відновлюється (або видаляється,
  якщо його не було) при відкаті. Контрольний знімок відкату пишеться під
  окремим іменем: раніше інструкція перезаписувала вихідний
  `snapshot-A-before.json`, після чого порівняння звіряло файл сам із
  собою й неминуче давало `[SUCCESS]`, нічого не довівши.

  **Маркер відсутності site-файла міг пережити прогін.** Повторний пілот
  у тому самому каталозі доказів успадкував би `.ABSENT` від минулого
  разу: резервна копія створювалась, але відкат усе одно йшов гілкою
  маркера й **видаляв** реальний site-файл замість відновлення. Тепер
  обидва артефакти прибираються на початку кроку 3 і взаємовиключні за
  побудовою, а відкат зупиняється, якщо в каталозі доказів їх нуль або
  два.

  Там же: відкат більше не падає, коли site-файла не було й пілот його не
  створив (звірка не відібрала жодного значення) — відсутність тут є вже
  потрібним станом, а не помилкою.

  **Відкат тепер працює і з точки «крок 5 впав».** Це найімовірніший шлях
  відмови: порівняння A ↔ B не зійшлось, крок 6 не виконувався, тож
  резервної копії primary-шару ще немає. Попередня редакція починала
  відкат саме з неї й обривалась, так і не прибравши змінений site-файл —
  невдалий пілот лишався активним. Тепер site-шар відновлюється першим і
  завжди, primary — умовно. Запис маркера відсутності зроблено
  термінальним і перевіреним до того, як оператор торкнеться site-файла.

  **Корінь цілого класу дефектів прибрано: каталог доказів тепер
  одноразовий.** Успадкований від попереднього прогону
  `BRAVO.config.backup` змусив би відкат перезаписати цілий поточний
  `BRAVO.config` застарілою копією, а успадкований маркер `.ABSENT` —
  видалити реальний site-файл. Замість чергової латки каталог створюється
  з міткою часу, без `-Force`, і перевіряється на порожність; чистка
  артефактів (яка сама мовчки ковтала помилки видалення) прибрана як
  непотрібна.
  ...
  **Резервні копії публікуються атомарно.** Обірвана копія (диск
  заповнився, збій ФС) під фінальним іменем виглядала б для відкату
  готовим бекапом, і відкат перезаписав би нею **справний** файл. Обидві
  копії тепер пишуться під `*.partial`, звіряються за SHA-256 з
  оригіналом і лише тоді перейменовуються — той самий підхід, що вже
  вживає `Write-BRAVODiscoveryBaselineTextAtomic`.
...
- **Розкатка більше не приймає prerelease мовчки (#152)** — гейт релізу
  ...
  **Корінь.** `Install-BRAVOServer.ps1` лише друкував `[УВАГА]`, коли
  ...
  рішення. Наслідок задокументований: LIMS-TOP тривало працював у
