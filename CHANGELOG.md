...
- **Формальна схема Configuration v2 — типи site-шару відмовляють
  закрито (#154, B2)** — `BRAVO.local.config` більше не може мовчки
  підсунути значення неправильного роду у відомий ключ.

  **Що було.** Шар перевизначень перевірявся лише на ШЛЯХ: невідомий
  top-level або батьківський вузол відхилявся, невідомий кінцевий
  сегмент приймався (рішення власника D3). ТИП значення не перевірявся
  взагалі, тому `'maintenanceSettings.Limits.MinimumFreeSpaceGB' =
  'двадцять'` доходив до ефективної конфігурації, і наслідок оператор
  бачив не як помилку конфігурації, а як дивну поведінку перевірки
  вільного місця.

  **Що стало.** Новий модуль `BRAVO.Configuration.Schema` будує формальну
  схему з КАНОНІЧНИХ ДЕФОЛТІВ (`Get-BRAVODefaultConfiguration`) —
  `Node | Boolean | String | Number | Array` для кожного з 271 листа, плюс
  рід елементів масиву й допустимість `$null`. Руками задано рівно те,
  чого з дефолту вивести неможливо: 9 листів `discoverySettings.*` (дефолт
  `$null` = «покластись на auto-discovery») і `ExcludedDrives` (дефолт
  `@()` не має елемента, з якого виводити рід). Лист із дефолтом `$null`
  без явного запису — помилка побудови схеми, а не мовчазна діра.

  Перевірка стоїть у канонічному конвеєрі рівно там, де її місце: після
  невиконуючого вилучення даних (B1) і ДО мерджу, тож недійсне значення
  не встигає стати ефективним. Повідомлення називає точний dot-шлях,
  включно з індексом елемента масиву.

  **Що навмисно НЕ змінено.** Політика невідомих ключів лишилась
  дослівною (D3: невідомий leaf приймається + попередження + метадані);
  явний `@()` і `$null`-семантика збережені; legacy `BRAVO.config`
  типами не перевіряється (він прибирається задачею B4, а перетворення
  його розбіжностей на відмову запуску було б зміною поведінки поза
  обсягом B2); діапазони, переліки допустимих значень і семантика шляхів
  лишились у своїх канонічних власників — замість копії тих правил є
  механічний guard, що звіряє роди задокументованого каталогу
  Configurator-а зі схемою.

  **Ризик для розгорнутих серверів.** Site-файл, який сьогодні містить
  значення неправильного роду у відомому ключі, після оновлення
  зупинить запуск із явним повідомленням замість того, щоб мовчки не
  діяти. Це і є мета зміни; перевірити наявні перевизначення можна
  `deploy/Get-BRAVOConfigSiteDelta.ps1` (#154, B0).

  Покриття: 12 перевірок `Schema/*` (серед них
  `AcceptsCanonicalConfiguration` — канонічні дефолти проходять власну
  схему; `CoversEveryCanonicalLeaf` — жодного листа без придатного роду;
  `UnknownLeafForwardCompatibilityIsPreserved` і
  `NodeOverrideIsTypeChecked`) плюс наскрізна
  `ConfigLoader/LocalOverrideWrongTypeFailsClosed`.

- **`[short]` не є прискорювачем типу у Windows PowerShell 5.1** — прибрано
  з `BRAVO.Configuration.Schema` (де він валив усе завантаження
  конфігурації на першому ж нечисловому значенні) і з
  `BRAVO.Configuration.DataFile`, де лишався латентним: у ланцюзі `-or`
  дефектний операнд обчислюється лише тоді, коли жоден ранній не
  істинний, тому дострокове замикання ланцюга ховало його від тестів.
  Визначення числового типу переведено на `System.TypeCode`, який не
  залежить від таблиці прискорювачів конкретної версії; у DataFile —
  повне ім'я `[System.Int16]`. Щоб клас не повернувся, додано правило
  `ci\Test-BRAVOForbiddenPattern.ps1`: `[short]`/`[ushort]` у коді
  комплекту — заборонений патерн (PSScriptAnalyzer його не ловить).

- **Порожня колекція в обов'язковому параметрі** — `BRAVO.Configuration.Schema`
  явно дозволяє її (`[AllowEmptyCollection()]`) на кожному обов'язковому
  параметрі, який може отримати колекцію. Windows PowerShell відхиляє
  порожню колекцію тим самим неявним контролем, що й `$null` та порожній
  рядок, а для цього модуля вона штатна: порожній перелік порушень на
  валідній конфігурації, дефолт-масив `@()` і щойно створена схема `@{}`
  на початку побудови. Перевірку значень це не послаблює — її виконує код
  функцій, а не прив'язка параметрів.

- **Повний `BRAVO_SELF_TEST.ps1` став дешевшим без втрати покриття
  (#157, фаза 1)** — прибрано рівно ту роботу, яка виконувалась кілька
  разів над тими самими файлами. Жодної перевірки не видалено, не
  пропущено й не переведено в selective-режим; повний канонічний прогін
  лишається режимом за замовчуванням.

  **Що саме було вузьким місцем.** Чотири статичні аналізи — порожній
  `catch` без пояснення, statement-keyword у позиції команди,
  half-formatted `-f`-конкатенація і credential-shaped рядкові літерали —
  кожен окремо відкривав файл, ПОВНІСТЮ його розбирав і **окремо обходив
  усе дерево AST**. Тобто чотири `Parser::ParseFile` і чотири `FindAll`
  по ~4.7 МБ коду.

  Домінував саме обхід: `FindAll` викликає PowerShell-scriptblock на
  КОЖЕН вузол дерева. Перша ітерація цього виправлення прибрала лише
  зайвий розбір — і виграшу на CI **не дала**, що це й довело. Тому
  тепер спільні обидва кроки: один розбір і один обхід на файл, а
  класифікація вузлів за типом відбувається вже без повторних проходів.
  Фільтри, тексти й ПОРЯДОК assert-ів не змінені.

  Окремо — власне джерело self-test (найбільший файл комплекту, ~1.3 МБ):
  його розбирали заново три структурні guard-и і ще тричі читали як
  текст. Додано memo на один AST і один рядок; ключ memo містить час
  модифікації й розмір файлу, тож зміна файлу під час прогону не віддасть
  застарілий розбір.

  AST комплекту навмисно НЕ кешується між файлами: він живе рівно один
  виток циклу, тому пікова пам'ять лишається такою ж, як була для одного
  проходу.

  Покриття: 4 нові перевірки `Perf/*`, серед них
  `SingleAstPassOverRepositoryFiles` — механічний guard, що єдиний
  AST-прохід і далі живить УСІ чотири аналізи (тобто «прискорити»
  видаленням перевірки не вийде), що обхід дерева в ньому рівно один і
  що цикл синтаксичної перевірки `Parser/<file>` лишається окремим;
  а також `OwnSourceMemoSurvivesAlternatingAccess` — регресія на наївне
  memo, де кожен getter скидає сусідній.
...
- **Зниклий компонент більше не зникає тихо (#158, етап 3)** —
  ...
  джерелу тихо випасти з backup set.

  **Що було зламано.** Baseline читав ЛИШЕ
  `BRAVO_SETUP.ps1 -Action Test -ValidateOnly`, і лише як необов'язкове
  попередження. Тобто захист від дрейфу діяв рівно тоді, коли оператор
  дивиться на екран, і не діяв у запланованому нічному прогоні — саме
  там, де неповна копія лишається непоміченою. Для інструменту
  резервного копіювання це найгірший клас відмови: прогін виглядає
  успішним, а компонента в копії немає.

  **Матриця рішення** (нова канонічна
  `Test-BRAVODiscoveryComponentDrift` у `BRAVO.Discovery`): `Present` —
  компонент активний; `Absent` без запису в baseline — INFO, легітимна
  відсутність; `Absent` із записом у baseline — ERROR; `Ambiguous` і
  `Error` — ERROR навіть без baseline («не можемо визначити» не
  дорівнює «немає»); компонент, вимкнений в конфігурації, — INFO і
  ніколи не дрейф (свідома політика оператора перевіряється ДО
  presence, тож вимкнений компонент не може зупинити прогін).

  **Перший запуск не ламається**: порожній baseline не містить жодного
  підтвердження, отже зникнути не могло нічого. Натомість **пошкоджений**
  baseline трактується як помилка, а не як «baseline немає»: мовчазний
  відкат на «перший запуск» знецінив би захист саме тоді, коли він
  потрібен.

  **Гранулярність.** Дрейф конкретного компонента вимикає саме його:
  решта архівуються, generation стає `INCOMPLETE`, прогін завершується
  кодом `30` (`InvalidConfiguration`) — той самий принцип, що вже
  застосований до SYSTEM access probe, і той самий механізм вимкнення,
  а не друга його копія. Скасування ВСЬОГО прогону лишилось за єдиним
  випадком, коли baseline узагалі неможливо оцінити. Зворотне рішення
  («будь-який дрейф скасовує все») означало б, що неоднозначний
  `BAZA_WWW` позбавляє резервної копії ще й `MODEL`/`BLOG`, тобто
  зменшує захист даних заради формальної суворості.

  Нового exit-коду не вводиться — `30` уже точно описує цей клас
  відмови. Нової команди підтвердження теж:
  `-ConfirmDiscoveryBaseline` лишається canonical механізмом і
  згадується в кожному повідомленні про дрейф разом з іменем
  компонента, попереднім джерелом і поточним presence-станом.

  Мапінг «увімкнені компоненти» став канонічним
  (`$global:discoveryEnabledComponents` у
  `BRAVO.Configuration.Derivation`): доти він будувався inline у
  `BRAVO_SETUP.ps1`, і Archive мусив би завести другу копію, яка
  розійшлася б із першою при першій же зміні складу компонентів. Мапа
  «компонент -> поле результату discovery» з
  `Test-BRAVODiscoveryResult` піднята на рівень модуля з тієї самої
  причини.

  Покриття: 8 регресій `Presence/*`, серед них
  `AbsentKnownComponentFailsClosed`, `AmbiguousFailsClosed`,
  `ErrorFailsClosed`, `ExplicitDisabledOverridesPresenceWithoutDriftError`,
  `FirstRunWithoutBaselineDoesNotTreatEverythingAsDisappeared`,
  `ConfirmBaselineAcceptsLegitimateChange`, `UnreadableBaselineFailsClosed`
  і структурний guard `DriftGateIsWiredIntoArchiveRuntimeAndSetup`.
  `docs/OPERATOR_CONSOLE_UX.md` §2.1 описує крок і матрицю.
...
- **Discovery baseline переїхав у машинний стан (#158, етап 1)** —
  ...
  `<RuntimeRoot>\LOGS\DISCOVERY_BASELINE.json`.

  **Чому це не косметика.** Baseline описує підтверджений оператором
  склад джерел, тобто це стан, а не журнал. У `LOGS` він означав, що чиста
  інсталяція комплекту в ІНШИЙ каталог лишає його позаду **разом із
  захистом, який він дає**: новий RuntimeRoot бачив «перший запуск», і
  будь-яке зникле джерело виглядало легітимно відсутнім. Для інструменту
  резервного копіювання це найгірший клас відмови — тиха неповнота.

  **Міграція автоматична й безпечна.** Якщо canonical-файлу немає, а
  старий є, його вміст переноситься **дослівно** (не перезбирається
  знімок: інакше міграція стала б новим підтвердженням baseline, якого
  оператор не давав). Файл у `LOGS` не видаляється — це дані оператора, а
  canonical виграє завжди.

  **Пошкоджений canonical лишається видимою проблемою** і не відкочується
  ні на legacy, ні на «перший запуск»: мовчазний відкат означав би
  «baseline немає» → «дрейфу немає» → зникле джерело знову виглядає
  нормою. `BRAVO_SETUP` друкує окремий рядок
  `Baseline discovery НЕПРИДАТНИЙ до читання`.

  Запис baseline став **атомарним** (тимчасовий файл → `[IO.File]::Replace`
  → прибирання в `finally`) — той самий патерн, що вже використовує
  `Write-BRAVOOperationStatus`; друга реалізація не вводилась. Розташування
  стану лишається за canonical `$global:stateRoot`
  (`Resolve-BRAVOConfigurationDerivation`), ACL — за
  `Protect-BRAVOMachineStateRoot`, який зміцнює весь `State`-корінь: жодна
  з цих політик тут не дублюється.

  Покриття: 9 регресій `DiscoveryBaseline/*`, серед них
  `BaselineLocationIndependentOfRuntimeRoot`, `MigrationPreservesContent`,
  `InvalidCanonicalFailsClosed` і структурний guard
  `SetupUsesCanonicalStatePath`.

  Обсяг етапу 1 навмисно вузький: **жодних змін у тому, які компоненти
  вмикаються**. Автоматичне enablement, `Absent + baseline => fail` і
  presence contract — наступні етапи #158.

- **Підсумок Maintenance більше не суперечить коду завершення (#175)** —
  прогін міг завершитись `Код завершення: 10` і водночас показати
  `Попереджень: 0`.

  **Корінь.** `Invoke-BRAVOMaintenanceDiskSpaceCheck` повертає `$true` і
  для успіху, і для non-blocking WARNING, а самі попередження пише через
  `Write-Log -Level WARNING` — тобто вони інкрементують
  `$script:BRAVOWarningCount`, і саме він дає exit 10. Крок «Перевірка
  вільного місця» при цьому штампувався сталим `'OK'` замість того, щоб
  брати статус із зрізу лічильників, як усі інші кроки, тож
  `$script:BRAVOMaintenanceStepWarnCount` (його читає підсумок) лишався
  нулем.

  **Виправлено двома рівнями.** Preflight-крок тепер бере статус з
  canonical `Get-BRAVOMaintenanceStepStatus` — тим самим патерном
  `$...CriticalBefore`/`$...WarningsBefore`, що вже застосований до
  «Зупинки служб», «Перевірки розмірів .md» та інших кроків. Додатково
  попередження, які не забрав ЖОДЕН крок (конфігураційна фаза, ділянка
  після останнього кроку), зводяться в один явний результат
  `Попередження поза кроками` — інакше та сама суперечність лишалася б
  досяжною поза preflight.

  Лічильник не накручується: текст попередження не дублюється (у зводі
  лише кількість), попередження, приписане крокові, у звід не потрапляє,
  а за їх відсутності запис не створюється зовсім. Арифметика підсумку
  лишається зімкненою (`Кроків` = `Успішно` + `Попереджень` +
  `Пропущено` + `Помилок`) — саме тому звід є окремим результатом, а не
  іншою формулою поля.

  Контракт exit-кодів, fail-closed поведінка preflight і формат
  структурованих рішень не змінені: зведення виконується ПІСЛЯ резолву
  коду завершення й на нього не впливає. Той самий звід застосовано і на
  шляху раннього виходу, який оминає фінальний блок.

  Покриття: 6 регресій `Maintenance/*` у
  `selftest/BRAVO_SELF_TEST.MaintenanceDiskSpace.ps1`, включно з
  `WarningAttributedToStepIsNotCountedTwice` (заборона подвійного обліку),
  `BlockingErrorIsNotMisclassifiedAsWarning` і структурним guard-ом проти
  повернення сталого `'OK'`.
- **Перелік required checks став перевірюваним, а не рукописним
  (#149)** — `RELEASE_POLICY.md` отримав §13.3 з канонічним переліком
  імен required checks, і self-test тепер механічно доводить, що кожна
  `pull_request`-задача `ci.yml` у ньому є
  (`Governance/RequiredChecksListCoversCiWorkflowJobs`), а жодне ім'я не
  має суфікса ` (push)`
  (`Governance/RequiredChecksListUsesPullRequestNames`). Саме цей дрейф
  і був коренем #149: `ci.yml` отримав задачу
  `BRAVO_DATA_RESTORE_MATRIX_TEST.ps1`, рукописний перелік §13 про неї
  не дізнався, і провалений E2E-тест відновлення лишався технічно
  немерджблокуючим.

  Перевірено за джерелом і зафіксовано в §13.3: задача виконується на
  КОЖНОМУ PR (тригер `pull_request: {}` без умов), креденціал 7z
  генерується в самій задачі, тож зовнішніх секретів вона не потребує й
  не пропускається, а її PR-ім'я стабільне (`BRAVO_DATA_RESTORE_MATRIX_TEST.ps1`,
  без суфікса). Тобто увімкнення її як required не може заблокувати PR
  «перевіркою, якої не буде».

  **Саме увімкнення в налаштуваннях GitHub НЕ виконано** — це
  адміністративна дія власника, і §13.3 прямо каже, що доти провалений
  матричний тест мердж не блокує. Документування контролю не є його
  впровадженням.

- **Точну конфігурацію tag ruleset задокументовано (#151)** —
  `RELEASE_POLICY.md` §16.2 тепер містить не опис наміру, а таблицю
  параметрів (`refs/tags/v*`, Restrict deletions + Restrict updates +
  Block force pushes, порожній bypass-list) і пояснення, чому порожній
  bypass-list принциповий. Кодова половина контракту (§16.1: заміна
  опублікованого ассета іншими байтами провалює workflow; регресія на
  форму виклику `gh release upload` у self-test) уже закрита раніше —
  незакритим лишається саме ruleset, і це теж адміністративна дія.
...
- **D2 вирішено: назва `BRAVO.local.config` лишається (#154)** — разом
  ...
  `DEFAULT < BRAVO.config < BRAVO.local.config`. Перейменування
  вимагало б власної міграції на всіх серверах без функціональної
  переваги, а третій шар не має підтвердженої потреби в парку. Рішення
  зафіксовано в `docs/design/BRAVO_CONFIGURATION_V2_COMPLETION.md`
  (блок із пріоритетом над рештою документа), `ROADMAP.md` і
  `TODO_FEATURES.md`. Секції design-документа про третій шар і матрицю
  режимів на три файли **свідомо не переписані**: коректна двошарова
  семантика конфліктів — це дизайн-робота B1–B3, а не заміна назви
  пошуком; натомість вони явно позначені як застарілі, щоб ніхто не
  реалізував скасоване. Зміни лише документаційні, runtime не
  торкнуто.

- **D3 вирішено: forward-compat для невідомого leaf зберігається (#154)** —
  вимогу Config v2 «відхилення невідомих ключів» звужено: невідомий
  top-level блок і невідомий батьківський вузол лишаються fail-closed,
  а невідомий **кінцевий сегмент** приймається. Суворість тут коштувала
  б сумісності з новішим Configurator: файл, згенерований новішою
  версією, відхилявся б старішим комплектом цілком, і оператор лишився
  б без конфігурації взагалі. Відповіддю на опечатку лишається
  **видимість**, а не відмова — попередження при завантаженні плюс
  `BravoConfigurationMetadata.LocalConfigUnknownLeafOverrides` (A2).
  Це не тимчасовий щабель до fail-closed, а кінцевий стан, і саме так
  тепер записано в `docs/design/BRAVO_CONFIGURATION_V2_COMPLETION.md`
  і `TODO_FEATURES.md`. Обсяг **B2** (формальна схема v2) звужується
  відповідно.

- **Документація leaf наздогнала поведінку (#154, A1, знахідка F2)** —
  після A2 невідомий кінцевий сегмент у `BRAVO.local.config` більше не
  мовчить: завантаження виводить попередження й записує ключі в
  `BravoConfigurationMetadata.LocalConfigUnknownLeafOverrides`. Але
  `BRAVO.local.config.example` і `BRAVO_SETUP.md` усе ще стверджували
  протилежне — «буде прийнято **мовчки**». Тобто документи відставали
  від коду **в інший бік**, ніж описувала знахідка F2: спершу вони
  перебільшували суворість, тепер — непомітність. Обидва вирівняно:
  прийом не змінився (forward-compat збережено), змінилася видимість, і
  саме це тепер і написано. `BRAVO_SETUP.md` додатково згадує
  діагностику primary-шару з A3
  (`PrimaryConfigIgnoredGlobals`, `PrimaryConfigUnknownNestedKeys`).
  README §10 крок 3 більше не відправляє оператора шукати site-правки
  вручну — він вказує на `deploy/Get-BRAVOConfigSiteDelta.ps1` (B0), який
  саме це й робить, із застереженням звіряти результат, а не копіювати
  наосліп. Наявний guard
  `ConfigLoader/LocalConfigLeafSemanticsDocumentedAccurately` посилено
  третьою умовою: обидва документи мусять згадувати
  `LocalConfigUnknownLeafOverrides`, інакше опис знову відстане від
  поведінки й цього ніхто не помітить.

- **Гейт паритету конфігурації знову прохідний (#154, A4,
  знахідка F4)** — `ci/Test-BRAVOConfigFoundationParity.ps1` падав на
  дев'яти полях, і жодне з них не було регресією. Чотири
  (`BuildId`, `PackageVersion`, `ScriptVersion`, `ScriptBuildId`)
  беруться з `VERSION.json`, а не з конфігураційного пайплайна, тож
  відрізняються між **будь-якими** двома комітами, де версію штампували:
  той самий клас, що вже внесений `LoadedAt`, просто пропущений при
  першому складанні allowlist-а. Через це harness ставав непрохідним
  після ПЕРШОГО ж штампування версії — а `RELEASE_POLICY` §14.4
  оголошує його обов'язковим для змін конфігураційного пайплайна, тобто
  гейт не міг дати PASS уже ні для чого. Решта п'ять
  (`componentSettings.SFTP.ArchiveLogUploadEnabled`,
  `.MaintenanceLogUploadEnabled`,
  `maintenanceSettings.Retention.RawSourceGraceDays`,
  `sftpDirectories.ArchivLog`, `.MaintenanceLog`) — адитивні ключі з
  коміту `5c14a70`: у базі `42cf9ad` їх не існувало взагалі, звідси
  `<null>` у BEFORE. Підтверджено прогоном harness на **чистому**
  `developer` без жодного PR: ті самі дев'ять. Ключі внесено
  **поіменно** — загальне правило «BEFORE=null, AFTER≠null — пропускаємо»
  послабило б перевірку, бо поле, яке раніше мало `$null`, а тепер має
  значення, є справжньою зміною поведінки й від адитивного ключа
  автоматично не відрізняється. `RELEASE_POLICY` §14.4 доповнено: база
  незмінна, отже allowlist зростатиме, і перед внесенням шляху треба
  довести адитивність. Заодно виправлено рекомендацію з #154
  («BaseRef = `origin/master` або тег»): вона не застосовна, поки stable
  лишається 5.2.4 — у `master` немає навіть
  `modules/BRAVO.Configuration/`.
- **Primary-шар більше не мовчить про ключі, яких не приймає (#154,
  A3, знахідка F1)** — асиметрія суворості між шарами була зворотною до
  очікуваної: `BRAVO.local.config` fail-closed на невідомий батьківський
  вузол і (з A2) звітує про невідомий leaf, а головний файл конфігурації
  мовчав в обох випадках. Невідомий top-level `$global:` просто не
  потрапляв у canonical allowlist і зникав безслідно; невідомий
  вкладений ключ мовчки зливався в ефективну конфігурацію, де його ніхто
  не читає. Оператор, що дописав поле в `BRAVO.config`, не мав жодного
  способу дізнатися, що воно ні на що не впливає. Тепер завантаження
  виводить попередження й записує обидва переліки в
  `BravoConfigurationMetadata.PrimaryConfigIgnoredGlobals` та
  `.PrimaryConfigUnknownNestedKeys`. **Прийом/відхилення не змінено** —
  як і в A2, це діагностика, а не новий fail-closed (сувора відмова
  лишається рішенням D3 у #154). Перелік оголошених файлом `$global:`
  будується з AST, а не зі знімка глобальної області: знімок дав би шум
  рантайму й пропустив би присвоєння вже наявній змінній. Перелік того,
  що встановлює сам канонічний pipeline, збирається там, де він і
  виникає (`Complete-BRAVOConfigurationLoad`), а не хардкодиться —
  інакше він застарів би при першій зміні деривації. Порівняння
  вкладених ключів — canonical `Compare-BRAVOConfigurationGraph`
  (#154, B0), без другого обходу графа. Комплектний `BRAVO.config` дає
  порожні обидва переліки (регресія
  `PrimaryStrictness/PristineConfigProducesNoDiagnostics`); усього шість
  регресій `PrimaryStrictness/*`.
...
- **Опублікований release-артефакт більше не можна мовчки замінити
  ...
  виконувався і для draft, і для опублікованого релізу. Повторний прогін
  workflow на вже опублікованому тезі замінював байти артефакту, який
  оператори вже могли завантажити, — контракт «один тег — один незмінний
  набір артефактів» не виконувався. Тепер `isDraft` вирішує: для draft
  заміна лишається (ремонтний сценарій), для опублікованого релізу
  збіг SHA-256 дає no-op, розбіжність — **FAIL** без заміни, а відсутній
  ассет доливається без `--clobber`. Звірка завантажує опублікований
  ассет і хешує його, а не довіряє `.sha256`-супутнику, бо супутник сам є
  ассетом. `RELEASE_POLICY.md` §16.1 фіксує контракт, §16.2 — вимогу
  ruleset для `refs/tags/v*` як окрему адміністративну дію.

- **SELF_TEST: стеля 4096 змінних на область (#163)** — усі 25
  фрагментів `selftest\*.ps1` dot-source'яться в ОДНУ область кореневого
  `BRAVO_SELF_TEST.ps1`, тож їхні змінні накопичуються разом і впираються
  в `$MaximumVariableCount` (типово 4096). Запас уже був меншим за 15
  змінних: блок з новими тестами завалив прогін помилкою
  `Cannot create variable ... capacity 4096 is exceeded` у **геть іншому**,
  не пов'язаному тесті (`Isolation/NoSelfTestOwnedDynamicModuleExports...`),
  через що причину шукали там, де її не було. Ліміт тепер піднімається
  явно на початку файлу, а наприкінці прогону друкується фактичне
  використання й перевіряється запас
  (`Framework/VariableScopeHeadroom`) — саме лише підняття стелі було б
  тією самою проблемою, відкладеною до 8192. Конвенцію для авторів
  тестів (виконувати блок фрагмента в дочірній області `& { ... }`)
  задокументовано там, де її побачить автор — у коментарі біля самої
  стелі.
...
на `LIMS-TOP` (ВІННИЦЬКА ФВЛ [38511934]). Зведені докази —
`docs/BRAVO_524_RC1_ACCEPTANCE_EVIDENCE_20260913.md`.

Промоція лише метаданих: жодної функціональної runtime-зміни відносно
`5.2.4-rc.1`. `.ps1`/`.psm1`/`.psd1` не змінювались, `ModuleVersion` усіх 18
модулів уже дорівнює базовій версії `5.2.4`.

### Результат acceptance

Чотири прогони архівації на томі `D:` (931.51 GB ємності, ~715 GB вільних,
сукупна потреба `AggregatedRequiredGB = 0.0686` GB):

| Поріг | `EstimatedSpaceMarginPercent` | Exit | Рішення |
|---|---|---|---|
| 20 GB | 25 | 0 | усе `Success` |
| 730 GB | 25 | 10 | `BelowHealthFloorButRequirementSatisfied`, `Blocks=False` |
| 20 GB | 2000 | 0 | усе `Success`, потреба 0.07 → 1.15 GB |
| 1000 GB | 25 | 0 | `BelowHealthFloorButRequirementSatisfied`, `Blocks=False` |

Кожен прогін — `COMPLETE`, 3 з 3 компонентів, SFTP 7 з 7, post-backup
health OK. Прогін з порогом 1000 GB окремо цінний: поріг перевищує повну
ємність тому, тобто недосяжний навіть на порожньому диску — під 5.2.3 це
був би вічний жорсткий блок. Жодного `BelowFloorEstimateNotPeakSafe` у
логах кандидата немає.

Артефакт прив'язано до CI: незалежно відтворений `git archive` з тега дав
той самий SHA-256, що й лог збирання (run `34755687320`). Self-test з
розпакованого артефакту — 1539 перевірок, 0 помилок.

### Межі доказу

Стеля за розміром джерела (`HistoryCappedBySource`) і bootstrap-компонент
без історії на сервері **не відтворені**: джерела там значно більші за
history-оцінку навіть із 20-кратним запасом, а всі три компоненти мають
валідну історію. Обидві гілки покриті юніт-тестами через реальний обхід
файлової системи.

### Знайдено під час acceptance, винесено в 5.3.0

Дві неблокуючі P2-позиції, кандидат через них не змінювався
(`docs/BRAVO_530_DISK_SPACE_HEALTH_SIGNAL_TASK.md` §3, §4):

- зведений операторський рядок рішення приписує шляхам причину першої
  entity тому — успадковано з 5.2.3, структурований лог правдивий;
- `MinimumFreeSpaceGB` лишився жорстким гейтом для Maintenance, тоді як
  для Archive став сигналом здоров'я; ключ у них спільний, тож підняття
  порогу заради архівації ламає обслуговування.

## 5.2.4-rc.1 — 2026-09-13

Hotfix-кандидат до 5.2.3. Одна змістовна зміна: **`MinimumFreeSpaceGB`
більше не блокує архівацію, якщо місця для самої операції вистачає.**

### Що було не так

5.2.3 перетворила поріг вільного місця з захисту здоров'я тому на гейт
операції. Наслідок відтворений на `LIMS-TOP` 13.09.2026: 715.46 GB
вільного, розрахункова потреба 0.0686 GB — і `exit 40`, бо поріг стояв
вище за вільне місце.

Обґрунтуванням 5.2.3 було «оцінку не доведено peak-safe», а названими
причинами — retained generations і `.work`. Жодна не витримує перевірки:
`.work` лежить на тому самому томі й публікується ПЕРЕЙМЕНУВАННЯМ
(`New-BRAVOTemporaryArchivePath`), тобто в піку тримає один розмір архіву,
а не два; наявні генерації вже враховані у виміряному `AvailableGB`, бо
вимір відбувається до створення нової. Реальною дірою було інше —
компонент без валідної історії взагалі випадав з оцінки.

### Виправлено

- **`RequirementPolicy` для Archive: `ArchiveNotPeakSafe` →
  `ArchivePeakSafe`.** Поріг знову є тим, чим задумувався: індикатором
  здоров'я тому. Below-floor при достатній вимозі дає WARNING
  (`BelowHealthFloorButRequirementSatisfied`) і ДОЗВОЛЯЄ прогін.
  Блокує лише невиконана вимога (`EstimatedRequirementNotMet`).
- **Компонент без історії більше не випадає з оцінки.**
  `Get-BRAVOArchiveEstimatedSpaceRequirement` виводить його вимогу з
  нестиснутого розміру джерела. Архів фізично не може бути більшим за
  своє джерело, тому `sourceBytes * (1 + SourceOverheadPercent/100)` —
  доведена верхня межа, а не прогноз. До 5.2.4 перший запуск компонента
  проходив узагалі без перевірки вимоги.
- **History-оцінка обмежується згори тією ж межею.** Якщо джерело
  зменшилось із часу останнього архіву, вимога зменшується разом з ним
  (`EstimateBasis = HistoryCappedBySource`). Стеля робить вимогу
  тіснішою і ніколи не більшою.

Порожнє або невимірюване джерело стелі не дає — інакше нульовий вимір
обнулив би вимогу компонента, який має що архівувати.

### Операційний вплив

Скасовує **обидва** попередження про розкатку з розділу 5.2.3 нижче в
частині блокування: сервер, який проходив поріг лише завдяки below-floor
relaxation у 5.2.1/5.2.2, після 5.2.4 знову проходить. `ExcludedDrives`
і далі не приховує нестачу місця на required volume — ця частина 5.2.3
лишається чинною і правильною.

Нових конфігураційних ключів немає. `SourceOverheadPercent` — параметр
функції зі значенням за замовчуванням 2%.

### Перевірка

Регресії: `Archive/A24` (below-floor при достатній вимозі ДОЗВОЛЯЄ —
інвертований відносно 5.2.3), `Archive/A25` (невиконана вимога блокує й
далі), `Archive/A26` (bootstrap несе вимогу з джерела),
`Archive/EstimatedSpaceBootstrapUsesSourceUpperBound`,
`…HistoryCappedBySmallerSource`, `…EmptySourceDoesNotZeroRequirement`.
Real-server acceptance (`RELEASE_POLICY.md` §9) — окремий крок,
обов'язковий перед промоцією у stable.
