...
- **Фатальна помилка більше не ховає решту прогону self-test.** Один виняток
  «The property 'Count' cannot be found on this object» обривав
  `BRAVO_SELF_TEST.ps1` на першому ж місці: оператор бачив один рядок
  `[FAIL] Fatal` замість повної картини, і жодна з наступних сотень
  перевірок не виконувалась.

  **Що змінилось.** Тіло прогону — кореневий скрипт і всі 26 доменних
  фрагментів — розбите на 125 послідовних try-секцій. Фатальна помилка
  тепер закриває ОДНУ секцію (у рядку `[FAIL]` є файл, номер рядка, текст
  винятку і сам рядок-винуватець), а прогін продовжується з наступної.
  Під запобіжник узято й післяпрогінний хвіст (gate ізоляції, прибирання,
  framework-перевірки): раніше виняток там убивав скрипт уже після
  останньої перевірки — без операторського підсумку й без коду завершення
  для CI.

  **Підсумок лишається fail-closed.** Кожна перервана секція додає запис у
  `$script:failures` (код завершення 1) і окремий рядок «Перервані секції»
  в операторському підсумку — за тим самим принципом, що й «Недоступно
  (хост)»: прогін, у якому частину перевірок не виконано, не має виглядати
  як повний.

  **Чому не всередині `Test-BRAVOCondition`.** Аргументи `-Condition`/
  `-Failure` обчислюються ДО входу у функцію (типовий випадок —
  інтерполяція `"$($x.Count)"` у тексті `-Failure`, яка виконується навіть
  коли перевірка проходить), тому перехопити виняток усередині функції
  неможливо.

  **Код самих перевірок не змінено.** Усі 1951 наявний виклик
  `Test-BRAVOCondition` лишився дослівно тим самим і в тому самому
  порядку: вставлено лише рядки меж секцій, без переформатування тіла. У
  фрагментах і у двох великих фікстурних блоках тіло секції навмисно
  лишається на тому самому рівні відступу, що й її `try` — зсув відступів
  змінив би вміст here-string'ів і сховав би реальні зміни в diff.

  **Нові перевірки `SelfTestGuards/*`** стережуть сам запобіжник: кожна
  перевірка виконується всередині секції, жодна секція не ховає понад 60
  перевірок, прогін лишається розбитим, а перервана секція справді
  реєструється й не зупиняє прогін (жива проба на `.Count` по `$null` під
  `Set-StrictMode`).
- **Ідентифікатори реальних серверів і установи прибрано з репозиторію.**
  ...
  **Що саме прибрано.** Назва production-сервера (16 файлів), назва
  установи разом із її кодом (7 місць у `CHANGELOG.md` і трьох
  документах `docs/`), внутрішні IP-адреси й **публічна** IP-адреса
  майданчика у фікстурах сповіщень. Останнє було найчутливішим: публічна
  адреса ідентифікує зовнішню точку входу.

  **Чим замінено.** Нейтральною роллю — «production-сервер парку» у прозі
  й «сервер парку» в коментарях коду, щоб не ламати ширину рядка. Роль
  зберігає технічний сенс кожного коментаря: чому цей тест існує і який
  реальний дефект він ловить, лишається зрозумілим без назви хоста.
  IP-адреси замінено на діапазони з RFC 5737 (`192.0.2.0/24`,
  `198.51.100.0/24`), призначені саме для документації.

  **Перший прохід був неповним — знайдено на рев'ю.** Пошук за точними
  рядками пропустив дві похідні форми: транслітерацію назви установи в
  іменах архівів (`<archivePrefix>_…`) і назву облікового запису завдань,
  утворену від імені сервера. Обидві прибрано; повторний скан веде пошук
  регістронезалежно й за транслітераціями, а не лише за кириличним
  написанням.

  **Межа, про яку треба знати.** Зміна стосується робочого дерева. Git-
  історія лишається недоторканою: прибрані рядки й далі видно через
  `git log -p`. Переписування історії не виконувалось свідомо — воно
  зробило б недосяжними `sourceCommit` усіх версій у `VERSION.json`,
  зламало б `BaseRef` parity harness і не перемістило б теги, захищені
  ruleset-ом.

- **Порівняння знімків ефективної конфігурації полагоджено — три дефекти,
  через які доказ міграції був недосяжним або хибним (#154).** Знайдено на
  рев'ю #203 уже після мержу.

  **Масив словників завжди звітував відмінність.** Гілка колекцій у
  `Test-BRAVOConfigurationValueEquality` рекурсувала в саму себе, а та
  **безумовно** повертала `$false` для будь-якого словника. Оскільки
  `$global:archiveDefinitions` — масив із трьох hashtable, два **ідентичні**
  знімки ніколи не могли зійтись: інструмент порівняння виходив з кодом 1
  завжди. Додано структурне порівняння словників усередині колекцій;
  приховати справжню відмінність воно не може — звіряються і набір ключів,
  і кожне значення. Верхньорівнева відмова для словника лишилась як була:
  там вона свідома.

  **Масиви втрачали форму при конвертації.** `return @(...)` віддає масив
  у pipeline, а pipeline його розгортає: одноелементний масив ставав
  скаляром, порожній — нічим. Тобто міграція, що перетворила `@('F:\')` на
  `'F:\'`, виглядала б у порівнянні **ідентичною**, і інструмент звітував
  би `[SUCCESS]` про зміну типу значення — рівно те, заради чого він існує.
  Виправлено поверненням `,$array`.

  **Приведення типів ховало справжні зміни.** Оператори PowerShell
  вважають `$true` рівним `1`, а `30` рівним `'30'`. Для доказу міграції
  це дірка: значення, що змінило тип, лишалось би «незмінним», і
  інструмент звітував би `[SUCCESS]`. Канонічний компаратор тепер звіряє
  КАТЕГОРІЮ типу перед значенням. Саме категорію, а не `GetType()`: усі
  цілі й дробові — одне число, інакше `Int32` проти `Int64` (звичайна
  різниця між значенням у пам'яті й ним же після JSON-циклу) давала б
  хибну відмінність на двох ідентичних знімках. Перевірка лише **додає**
  відмінності й ніколи не ховає наявних — безпечний напрямок; побічно
  вона робить суворішим і `deploy\Get-BRAVOConfigSiteDelta.ps1`, який
  раніше не показував оператору зміну типу site-значення.

  **Відкат пілота більше не падає на відсутньому site-файлі.** Якщо
  звірка на Кроці 2 не відібрала жодного значення, `BRAVO.local.config`
  не створювався взагалі — і гілка маркера `.absent` викликала
  `Remove-Item -ErrorAction Stop` на файлі, якого немає. Відкат обривався
  вже ПІСЛЯ відновлення primary-шару й ДО верифікації SHA-256, хоча
  відсутність тут є вже потрібним станом, а не помилкою. Додано
  `Test-Path` і явну зупинку на випадок, коли в `$backupDir` немає ані
  копії site-файла, ані маркера його відсутності: раніше така
  неоднозначність мовчки лишала site-шар у мігрованому стані.

  Решту дефектів runbook, знайдених на тому ж рев'ю, вже закрив
  незалежний перегляд Кроку 2а (#207): резервна копія обох шарів до
  першого запису, одноразовий каталог доказів і верифікація відновлення
  за `backup-manifest.json`.

  Регресії: `Delta/ArraysOfDictionariesCompareStructurally` — з двома
  позитивними контролями (змінене значення й зайвий ключ мусять лишатись
  видимою відмінністю); `Delta/TypeChangeIsNotHiddenByCoercion` — чотири
  позитивні випадки плюс негативний контроль `Int32`/`Int64`, без якого
  фікс «полагодив» би проблему ціною завжди червоного порівняння.
- **Ефективна конфігурація отримала канонічний знімок і доказове
  порівняння (#154, підготовка pilot migration)** — новий доменний модуль
  `BRAVO.Configuration.Snapshot`, прапорець `BRAVO_CONFIG_TEST.ps1
  -FullGraph` і `deploy\Compare-BRAVOConfigEffectiveSnapshot.ps1`.
  **Поведінка наявних шляхів не змінюється**: без `-FullGraph` вивід
  `BRAVO_CONFIG_TEST.ps1` той самий, що й раніше.

  **Навіщо.** Критерій приймання міграції site-значень — «ефективні
  значення не змінились» — був недоказовим. Ефективна конфігурація це 77
  іменованих `$global:`-змінних, а єдиний серверний інструмент показував
  п'ять із них (корені + режим завантаження). Parity harness бачить
  повний граф, але він git-залежний і підмінює `LIMSRoot`/`BackupRoot`
  фікстурними шляхами — на сервері не запускається взагалі. Тобто пілот
  можна було виконати, але не можна було **прийняти**.

  **Перелік імен переїхав до канонічного власника.** Він жив літералом
  усередині `ci\Test-BRAVOConfigFoundationParity.ps1`; поки споживач був
  один, це було нормально. Тепер споживачів два, і дві копії переліку
  означали б, що доказ паритету в CI і доказ міграції на сервері мовчки
  дивляться на різні графи. Перелік і порядок імен перенесено **без
  змін**, тож поведінка harness-а збережена; `Snapshot/ParityHarnessUsesCanonicalList`
  не дає повернути літерал назад.

  **Механіка зчитування в harness лишилась дубльованою — свідомо.**
  BEFORE-бік виконується в дереві `42cf9ad`, де цього модуля ще не існує;
  імпортувати модуль з AFTER-дерева в BEFORE-захоплення означало б внести
  код «після» у знімок «до», тобто знецінити сам characterization-тест.
  Спільним лишився тільки перелік імен, який параметризується ззовні.

  **Відсутнє поле відрізняється від `$null`.** Маркер `<<ABSENT>>` — рядок,
  а не `$null`, бо `$null` є легальним значенням конфігурації: інакше
  зникнення значення читалося б як «поле дорівнює `$null`» і проходило б
  як PASS.

  **Порівняння не заводить другої політики.** Обхід графа виконує
  канонічний `Compare-BRAVOConfigurationGraph`; у `deploy`-скрипті лише
  нормалізація JSON у hashtable (`ConvertFrom-Json -AsHashtable` у Windows
  PowerShell 5.1 немає) і поіменна класифікація полів, які міграція
  змінює **за визначенням** — вони описують джерело конфігурації, а не
  значення. Вузол метаданих навмисно не заглушено цілком: версія
  комплекту в ньому якраз мусить лишитись незмінною.

  Процедура пілота — `docs/BRAVO_CONFIG_V2_PILOT_MIGRATION_RUNBOOK_20260916.md`.
  Доказом міграції є порівняння A ↔ C (до міграції проти стану без
  `BRAVO.config`), а не A ↔ B. **На жодному сервері ще не виконувалась.**

- **Шлях до legacy `BRAVO.config` у self-test має одного власника
  (#154, підготовка до B4-2)** — `Get-BRAVOSelfTestLegacyConfigPath` /
  `Get-BRAVOSelfTestLegacyConfigText`. **Поведінка не змінюється ані на
  байт**: повертається той самий кореневий файл.

  **Навіщо.** 19 місць у кореневому наборі й у п'яти фрагментах
  незалежно будували `Join-Path $root 'BRAVO.config'` — частина як базу
  для синтетичної фікстури, частина для статичних тверджень про вміст
  комплекту. Саме цей розсип і робив крок B4-2 («прибрати файл з
  пакета») заблокованим: він означав переписати фікстурну тканину
  **одночасно** зі зміною runtime-контракту й одночасно зі зламом
  parity harness — тобто три ризики в одній зміні.

  Тепер шлях знає рівно одна функція. Коли B4-2 прибере файл із пакета,
  зміниться **одне тіло функції**, а не 19 місць.

  **Чому не заморожена копія вже зараз.** Доки файл є в пакеті, фікстури
  мають читати саме його — інакше вони перестануть характеризувати те,
  що реально відвантажується, а розбіжність між копією й оригіналом
  ніхто не помітить. Копія — крок B4-2, не цей.

  **Класифікацію довелося поширити й на кореневий файл.** Перша редакція
  розвела клас B лише у фрагментах; у `BRAVO_SELF_TEST.ps1` п'ять
  тверджень про пакет (`ConfigSecurity/RepositoryConfigIsStrict`,
  перевірки retention, discovery, size-sanity і спільний
  `$bravoConfigText`) далі йшли через фікстурний accessor. Переведено всі
  п'ять. Клас A в корені лишився рівно двома фікстурами.

  Тепер **три** канонічні власники, кожен зі своєю долею на B4-2:
  legacy-фікстури → заморожений актив, конфігурація пакета → канонічні
  дефолти, операційний `-ConfigPath` → лишається як є. Тіла
  accessor-ів навмисно не делегують одне одному й текстуально різні:
  делегування відтворило б пастку, а однакові рядки guard не розрізнив би.

  **Заміна присвоєння зламала сусідній guard — виправлено.** Guard
  `Health/SelfTestDoesNotModifyAcl` шукав кінець dev.13-блоку за текстом
  присвоєння `$bravoConfigText = [IO.File]::ReadAllText(`. Коли це
  присвоєння перейшло на accessor, `IndexOf` почав знаходити **власний
  рядковий літерал цього ж guard-а** нижче за текстом, блок розтягувався
  на коментарі з назвами `Set-Acl`/`AddAccessRule`/`FileSystemAccessRule`
  — і guard падав детерміновано, у кожному прогоні. Межу переведено на
  виділений маркер-коментар `# END-MARKER: dev.13 ACL guard block
  boundary`, який не є кодом і тому не залежить від рефакторингу сусідніх
  присвоєнь. Перевірено симуляцією семантики `String.IndexOf` на
  фактичному файлі: до правки блок містив усі чотири заборонені назви,
  після — жодної, довжина блоку збереглася.

  Обіцянка «зміниться рівно одне тіло функції» стосується **споживачів**
  accessor-ів. Сам guard `Governance/LegacyConfigPathHasSingleOwner` на
  B4-2 **заплановано впаде**: перелік легальних місць читання кореневого
  `BRAVO.config` звузиться до одного, і його доведеться звузити й у
  guard-і. Це не прихована зв'язаність, а навмисна вимога рев'ю на зміну
  цього переліку.

  **Guard звіряв ім'я файлу регістрочутливо — дірка закрита.** Цільова ФС
  регістру не розрізняє, тож `Join-Path $root 'bravo.config'` читався б
  успішно, а статичні перевантаження `[regex]::Matches`/`IsMatch` за
  замовчуванням регістрочутливі — guard звітував би PASS при живій прямій
  залежності. Ім'я файлу тепер звіряється через `(?i:BRAVO\.config)` з
  обов'язковою межею `(?![A-Za-z0-9_])`, інакше збіг ловився б усередині
  `BRAVO.Configuration`. Порівняння рядків збігів переведено з `-eq` на
  `-ceq` з тієї самої причини.

  **Ім'я змінної навмисно лишилось регістрочутливим.** Прапорець `(?i)` на
  весь шаблон дає четвертий збіг —
  `$fixtureConfigPath = Join-Path $Root 'BRAVO.config'` у
  `New-BRAVOProductionConfigFixtureResult`, де `$Root` — **параметр
  функції** (тимчасовий fixture-корінь), а не корінь репозиторію. Тобто
  guard падав би на коректному коді. Конвенція файлу: `$root` —
  репозиторій, `$Root` — локальний параметр фікстури.

  **Твердження про пакет отримали власний accessor.** Клас B
  (`DiskSpace`, `Paths`, `BazaSync` — «у конфігурації немає
  `DiskSpacePolicyMode`» тощо) перевіряє **живий** стан того, що
  відвантажується, тоді як фікстурний accessor після B4-2 навмисно
  почне вказувати на **заморожений** актив. Якби вони ділили одну
  функцію, зміна її тіла мовчки перетворила б ці твердження на перевірку
  мертвого файлу: додавання `DiskSpacePolicyMode` у канонічні дефолти
  проходило б, доки заморожений актив без змін. Тому
  `Get-BRAVOSelfTestShippedConfigText` — окрема функція з тим самим
  тілом сьогодні й іншою долею на B4-2.

  **Guard звіряє не лише кількість, а й самі рядки збігів.** Двох
  входжень замало: прибрати одне легальне посилання й додати пряме
  читання деінде — лічильник лишився б 2, а guard звітував би PASS.

  **Дефолт `-ConfigPath` лишається окремим виразом.** Спершу я завів його
  на той самий accessor — і це була помилка проєктування: з `$ConfigPath`
  виводиться `$configRoot`, поруч з яким мусить лежати
  `BRAVO_CONFIG_LOADER.ps1`. На B4-2 accessor почав би повертати
  заморожений актив із `selftest\`, і весь набір упав би з
  «Configuration loader not found» ще до запуску suite-ів. Це різні
  відповідальності, тому в кореневому файлі два легальні посилання, і
  guard знає про обидва.

  Guard дивиться на **дві** форми прямої залежності: `Join-Path` і
  інтерполяцію кореня разом з іменем файлу. Друга форма пройшла повз
  першу редакцію guard-а — у `ConfigLoader` команда дочірнього процесу
  будувала шлях через вкладений вираз з екрануванням лапок, перевірка
  звітувала PASS, а пряме читання кореневого файлу лишалось. Знахідка
  code-review; точку переведено на accessor, шаблон розширено.

  Guard `Governance/LegacyConfigPathHasSingleOwner` тримає володіння: у
  `BRAVO_SELF_TEST.ps1` допускається рівно одне входження (тіло
  accessor-а), у фрагментах — жодного.

  Окремо лишається `ci\Test-BRAVOConfigFoundationParity.ps1`: він читає
  `BRAVO.config` з кореня **кожного** порівнюваного комплекту, що
  правильно за побудовою, і потребує власного рішення на кроці B4-2.

- **Провенанс артефакту звіряється за вмістом (#199)** —
  `ci\New-BRAVOReleaseArtifact.ps1` перед збиранням вимагає, щоб між
  `sourceCommit` і комітом спакованого ref-а відрізнялися **рівно**
  `VERSION.json` і `RUNTIME_MANIFEST.json`.

  **Корінь.** Форма `sourceCommit` і рівність `packageVersion` нічого не
  кажуть про вміст, а перештампування однієї версії — штатна практика
  (`5.1.0-dev.1` штампували 25 разів). Тому провенанс, залишений від
  попереднього штампу **тієї самої** версії, проходив усі наявні
  перевірки, хоч уже не описував спакований код.

  **Чому на шляху артефакту, а не в гейті PR.** Інваріант виконується
  лише на комітах-штампах: `developer` рухається, і після кожного мержу
  різниця проти `sourceCommit` законно стає сотнями файлів — в історії
  таких проміжних станів 73. У per-PR гейті перевірка була б
  непрохідною; на шляху тегу — точною.

  Задача збирання артефакту отримала `fetch-depth: 0`: `sourceCommit` —
  це батько коміта-штампа, а тег вішається на штамп, тож при
  shallow-checkout потрібного коміта немає. Додано й звірку типу
  об'єкта — `sourceCommit` має бути комітом, а не деревом.

  `ci\New-BRAVOReleaseArtifact.ps1` отримав параметр `-RepositoryRoot`
  (типово — той самий репозиторій). Без нього перевірки провенансу
  неможливо прогнати на ізольованій фікстурі, а тестувати їх
  переписуванням тієї самої логіки в тесті безглуздо. Той самий мотив,
  що й `-Root` у `ci\Test-BRAVOReleasePolicy.ps1`.

  Регресії `ReleaseArtifact/RejectsStaleProvenanceContent` і
  `ReleaseArtifact/AcceptsCleanStampProvenance` — behavior-level, на
  справжньому тимчасовому git-репозиторії. Маркери `PROVENANCE_STALE` і
  `PROVENANCE_OBJECT` свідомо ASCII: stderr дочірнього процесу
  кодується кодовою сторінкою консолі.

- **Провенанс `5.3.0-dev.3` виправлено, і тепер його звіряє CI (P0-B)** —
  `VERSION.json` перештамповано на `b9bd4dc`, додано перевірку провенансу
  в `ci\Test-BRAVOReleasePolicy.ps1`, `fetch-depth: 0` для задачі
  `static-checks` і guard, що ця опція не зникне.

  **Корінь.** Процедура штампування (`ci\Update-BRAVOVersionStamp.ps1`)
  має чотири кроки: коміт коду релізу з новою `packageVersion` → штамп на
  чистій копії → коміт-штамп із маніфестом → тег. Для `5.3.0-dev.3`
  виконався лише перший, та й той усередині merge-резолюції PR #143.
  Пакет заявляв `5.3.0-dev.3`, а `sourceCommit` `86270a1` веде на
  `VERSION.json` з `5.3.0-dev.2`.

  **Наявний guard обіцяв саме це й не робив.** Коментар
  `Version/StampConsistency` каже «ловить неузгоджений/pre-stamp
  VERSION.json (packageVersion нова, а build/sourceCommit від іншого
  коміту)», але реалізація перевіряє лише, що `buildId` — префікс
  `sourceCommit`. Тут це виконувалось.

  **Інваріант перевірено на всій історії, а не вигадано.** Прогін по всіх
  195 станах `VERSION.json` у репозиторії: на **кожному** коміті-штампі
  `packageVersion` у `sourceCommit` збігається з поточною. Усі 73
  розбіжності — проміжні коміти кроку 1, кожен закритий наступним
  комітом-штампом. Серед них був і `developer`, який пробув у цьому стані
  три доби.

  **Перевірка git-залежна, тому живе в CI, а не в self-test**: §14.4
  вимагає, щоб `BRAVO_SELF_TEST.ps1` лишався git-незалежним. Git-вільна
  половина (`buildId` як префікс) там і лишилась.

  **`sourceCommit` має бути комітом, а не будь-яким об'єктом.** `git show`
  приймає будь-який tree-ish, тому 40-символьний ID **дерева** з підхожим
  `VERSION.json` у корені проходив і перевірку форми, і `buildId`-префікс,
  і саме читання файлу — провенанс, що не вказує на жоден коміт. Тепер тип
  звіряється через `git cat-file -t`.

  **Репозиторій шукається лише в корені комплекту.** `git rev-parse`
  веде пошук угору по батьківських каталогах, тому розпакований комплект,
  що випадково лежить усередині чужого робочого дерева, вважався б
  репозиторієм — і його `sourceCommit` звірявся б із **чужою** історією.
  Тепер перевіряється `--show-toplevel`: провенанс звіряється тільки якщо
  корінь репозиторію збігається з коренем комплекту.

  **Недосяжний `sourceCommit` — це відмова, а не попередження**, якщо
  репозиторій поруч є. Інакше вигаданий 40-символьний hash із узгодженим
  `buildId` проходив би всі перевірки провенансу, і `fetch-depth: 0`,
  доданий саме заради авторитетності звірки, не давав би нічого. Виняток
  лишається один — розпакований комплект без `.git`.

  **Перевірки, які не залежать від гілки, більше не пропускаються разом
  із гілковими.** Раніше невідома гілка (detached HEAD, розпакований
  архів) давала `exit 0` ще до звірки `releaseDate` з `CHANGELOG.md` —
  тобто рівно в тому сценарії, який §5.3 називає підтримуваним без
  `.git`. Тепер пропускається лише відповідність «гілка — версія —
  канал».

  Два guard-и на `ci.yml` тепер шукають саме крок `run:`, а не згадку
  шляху будь-де у файлі: пояснювальний коментар біля `fetch-depth: 0`
  містить той самий шлях, тому підрядковий пошук лишався б зеленим і після
  видалення справжнього кроку — перевіряв би наявність коментаря про
  перевірку замість самої перевірки.

  Регресія `ReleasePolicy/RejectsProvenanceFromDifferentVersion` —
  behavior-level: піднімає тимчасовий git-репозиторій, відтворює рівно
  стан `dev.3` і доводить, що гейт його відхиляє. Якщо git недоступний,
  перевірка звітує `[НЕДОСТУПНО]`, а не тихо зеленіє.

- **`RELEASE_POLICY.md` більше не дашборд (P0-D)** — розділ 20 перейменовано
  на «Історія релізів». Рядки `master: 5.2.0` / `developer: 5.3.0-dev.2`
  велися вручну і відстали на чотири stable-релізи (5.2.1-5.2.4). Замість
  них — таблиця машинних джерел істини; хронологія прийнятих релізів
  лишається, бо це докази acceptance, а не стан.

  Там само зафіксовано **власника кожного поля `VERSION.json`**. Головне,
  що встановлено доказом: `releaseDate` належить **версії, а не штампу** —
  `5.1.0-dev.1` штампували 25 разів, дати комітів `sourceCommit`
  охоплюють 2026-08-11…08-14, а `releaseDate` весь час лишалась
  `2026-08-12`. Тому робити штампування автором цього поля було б
  регресією.

- **`deploy/README.md` більше не наводить історичний RC як приклад (P0-E)** —
  `v5.3.0-rc.1` у прикладі prerelease-розкатки замінено на плейсхолдер
  `vX.Y.Z-rc.N`. Той кандидат приймання не проходив, а його GitHub Release
  лишається draft-ом — за тим тегом артефакт узагалі не завантажиться.
...
- **Аналіз долі тега `v5.3.0-rc.1`** — новий документ
  ...
  **Дрейф навколо тега — потребує дій.** Аналіз зафіксував чотири знахідки, з
  ...
  На початку документа — таблиця стану знахідок: усі чотири закрито
  (П1 у #195, П2-П4 у #196, П2 додатково посилено в #197), тож читати їх
  як перелік відкритих проблем не треба. Сам аналіз лишено без змін — це
  запис того, чому й на підставі чого ухвалювались рішення.
...
- **Parity harness конфігурації виконується автоматично** — новий workflow
  ...
  конфігураційного пайплайна.

  **Корінь.** `RELEASE_POLICY.md` §14.4 називає цей harness **обов'язковим**
  доказом для таких змін, але виконувати його треба було вручну — і за
  шість merged PR конфігураційного пайплайна (#181, #183, #184, #185, #186
  і серію до них) він не запускався **жодного разу**. Обов'язок, який
  тримається на людській дисципліні й не має механізму, не виконується.

  **Перешкод не було.** Скрипту потрібні лише `git` і Windows PowerShell
  5.1: ні адмін-прав, ні служб, ні 7-Zip/WinSCP, ні мережі, ні
  встановленого LIMS (шляхи підмінено на фікстури всередині самого
  скрипта). Бракувало самого workflow. Єдина нетипова вимога —
  `fetch-depth: 0`: harness створює ізольований `git worktree` за
  незмінним комітом-базою, який при shallow-checkout недосяжний.

  **Окремий файл, а не задача в `ci.yml`** — з двох причин. Перевірка
  доречна лише для PR, що торкаються конфігурації, а фільтр `paths` у
  GitHub Actions задається на рівні workflow, не задачі. І guard
  `Governance/RequiredChecksListCoversCiWorkflowJobs` вимагає, щоб кожна
  `pull_request`-задача `ci.yml` була перелічена в §13.3 як required
  check — додавання туди сьомої задачі означало б заявити required check,
  якого насправді не ввімкнено, тобто відтворити дефект #149.

  **Це ще не required check**: наявність задачі в CI не робить її
  блокуючою. Промоція — свідоме рішення власника після того, як прогін
  стабільно зелений.

- **Вибірковий прогін self-test для розробника (#187, фаза 2)** —
  `BRAVO_SELF_TEST.ps1 -Suite DiskSpace,ArchiveDiskSpace` виконує лише
  названі suite-фрагменти. Повний канонічний прогін лишається режимом за
  замовчуванням і **єдиним**, що є gate-ом мержу й релізу.

  **Чому inline-тіло кореня виконується завжди.** Фрагменти не
  самодостатні: вони споживають фікстури, які готує саме воно —
  `$archiveScriptText` (11 вживань в `ArchiveDiskSpace`, жодного власного
  присвоєння), `$archiveRuntimeModuleText`, `$resolvedConfig`,
  `$backupRootPath`, `$statePath`. Пропустити корінь означало б упасти під
  `Set-StrictMode` на невизначеній змінній, і виглядало б це як дефект
  обраного фрагмента, а не як обмеження режиму — рівно той клас плутанини,
  який закриває #188. Тому економія обмежена зверху: корінь це 961 з 2107
  перевірок і 188.6 с із 477.5 с сумарного часу suite-ів, і обіцяти
  більше було б неправдою.

  **Вибірковий прогін технічно не може стати доказом релізу.** Він друкує
  `SELF-TEST PARTIAL: <фрагменти>` замість `SELF-TEST PASSED`;
  `RELEASE_CHECKLIST.md` вимагає саме дослівний `SELF-TEST PASSED`, тому
  межа тримається механізмом, а не домовленістю. Підсумок додатково
  показує рядок «Режим: вибірковий» і прямо каже, що для мержу потрібен
  повний прогін.

  **Невідоме ім'я зупиняє прогін** із переліком доступних: мовчки виконати
  не те, що просили, гірше, ніж не виконати нічого.

  Підказка «змінений файл → suite» (`Get-BRAVOSelfTestSuiteForChangedPath`)
  свідомо мінімальна: точна відповідність для фрагмента й доменного
  модуля, порожньо для всього іншого. Порожньо означає «виконуй повний
  прогін», а не довільний suite — застаріла мапа гірша за її відсутність,
  бо тихо радить пропустити те, що саме й зламано.

  Покриття: 12 перевірок `Framework/*`, серед них
  `SuiteCatalogMatchesGatedFragments` і `EverySuiteFragmentIsGated`
  (каталог і фактичні сайти підключення не можуть розійтися),
  `SelectiveRunNeverPrintsReleaseMarker`, `CiWorkflowNeverNarrowsSelfTest`
  (required check мусить лишатися повним), `FullCanonicalRunIsDefault` і
  `ChangedPathHintNeverGuesses`.

  Паралелізацію свідомо не зроблено: спільний стан між фрагментами не
  усунуто, і #187 прямо забороняє її до цього.

  Не перевірено локально: у середовищі немає Windows PowerShell.

- **Обмеження хоста в self-test більше не виглядає як дефект комплекту
  (#188)** — `BRAVO_SELF_TEST.ps1` отримав третю категорію результату.

  **Корінь.** Частина перевірок виконується в дочірньому процесі
  PowerShell (пауза `Start-Transcript`: Windows PowerShell 5.1 тримає
  лише один transcript на сесію). На RDSH-сервері така проба не
  стартувала, і `HelperLogging/SuspensionHidesConsoleOutput` та
  `HelperLogging/CanaryDetectsIneffectiveSuspension` давали `[FAIL]` з
  текстом «проба не повернула JSON». Оператор не мав способу відрізнити
  зламаний комплект від хоста, який не дає пробі працювати.

  **Причина відмови більше не зникає.** Помилка дочірнього процесу йшла
  в `stderr` батька і не потрапляла нікуди. Тепер проба звітує власну
  помилку в `stdout` (`ProbeError`), а батько додає до повідомлення код
  виходу й останні рядки виводу. Це закриває перший крок задачі —
  діагностику — навіть там, де класифікація не спрацює.

  **Третя категорія — `[НЕДОСТУПНО]`.** Не потрапляє в перелік помилок,
  тому не змінює код завершення; саме тому підсумок показує її окремим
  рядком і прямо каже «прогін НЕ повний» замість «усі перевірки
  пройдено».

  **Класифікація fail-closed.** `[НЕДОСТУПНО]` з'являється лише коли
  обмеження хоста прочитано як **факт** (`LanguageMode != FullLanguage`
  або `EnableTranscripting = 1`). Немає доказу — лишається `[FAIL]`:
  класифікувати дефект комплекту як обмеження середовища небезпечніше,
  ніж навпаки. Проба, яка відпрацювала і дала неправильний результат, —
  завжди `[FAIL]`, хоч би як налаштований хост.

  **Рішення винесено в чисту функцію** `Get-BRAVOSelfTestAssertionResult`:
  інакше та сама умова жила б і в гілці друку, і в телеметрії `Result`, а
  перевірити її можна було б лише справжнім викликом assert-а — тобто
  друком фікстурних `[FAIL]` у чистому прогоні, тобто рівно тим шумом,
  проти якого ця задача.

  Покриття: 9 перевірок `Framework/*`, серед них
  `BlankRestrictionNeverSilencesFailure` (порожня причина не глушить
  дефект), `RestrictionDoesNotOverrideSatisfiedCondition`,
  `HostRestrictionAbsentOnUnrestrictedHost` (детектор не вигадує обмежень
  там, де їх немає) і структурний
  `RestrictionReasonComesFromDetectorNotLiteral` — причина мусить
  надходити від детектора, а не рядковим літералом у місці виклику,
  інакше автор тесту міг би оголосити будь-який провал «обмеженням
  хоста».

  `OPERATIONS.md` описує всі три категорії й команди підтвердження
  причини на хості.

  Не перевірено локально: у середовищі немає Windows PowerShell.

- **RC-матриця приймання по ОС (#153)** — промоція RC → stable тепер має
  контракт доказів з реальних хостів, а не лише «щонайменше один
  сервер».

  **Корінь.** `RELEASE_POLICY.md` не містив формальної матриці ОС:
  єдиною згадкою була епізодична нотатка про acceptance 5.2.x
  (Server 2022 / хост Server 2016 LegacyBestEffort). Підстава емпірична —
  обидва дефекти, виправлені в PR #148, **неможливо було знайти в CI**:
  хибна вимога `diskshadow.exe` потребувала `bravo.ini` зі змішаним
  регістром **і** клієнтської Windows, а `The network path was not found`
  по недосяжному UNC відтворюється лише на доменному хості, де ім'я
  резолвиться.

  **Новий розділ §9.4.** Дві обов'язкові цілі (Windows Server 2022 і
  Windows 11, обидві з Windows PowerShell 5.1) і дві best-effort
  (Server 2016, Server 2012 R2). Клієнтська Windows у обов'язкових не
  випадково: саме на ній немає `diskshadow.exe`, і саме там знайдено
  перший дефект PR #148. Мінімальний набір сценаріїв на хост: архівація
  з VSS, обслуговування, завдання Планувальника від `SYSTEM`, self-test.

  **Матриця не вводить нової класифікації ОС.** Рівні лишаються за
  `modules\BRAVO.Compatibility` і `SECURITY.md`; §9.4 каже лише, що саме
  треба довести. Приймання по матриці — вимога промоції RC → stable, не
  gate на кожен PR.

  **Недоступність self-test класифікована окремо.** На жорстко
  налаштованому хості (GPO-транскрипція, Constrained Language Mode)
  дочірні проби не можуть виконатись — зафіксовано на RDSH-сервері, де
  `HelperLogging/*` дали `[FAIL]`, бо проба не повернула JSON. Такий
  результат не є ні провалом приймання, ні доказом справності:
  фіксується як `НЕДОСТУПНИЙ` із причиною. Класифікація цього стану
  самим harness-ом — окрема задача.

  **Матриця не осиротіла:** критерії готовності RC (§9.3) тепер прямо
  вимагають докази по §9.4.

  Покриття: 6 перевірок `Governance/*` — серед них
  `OsAcceptanceMatrixIsWiredIntoRcReadiness` (матриця, на яку ніхто не
  посилається, — мертвий текст), `OsAcceptanceMatrixReusesCompatibilityTiers`
  (заборона паралельної класифікації) і
  `OsAcceptanceMatrixExcludesUnsupportedSystems` — ціль приймання з рівня
  `Unsupported` неможлива за побудовою, бо production-запуск там
  блокується кодом `30`.

  Не перевірено локально: у середовищі немає Windows PowerShell.

- **Застарілий `BRAVO.config` перестав бути мовчазним (#154, B4 — частина 1)**
  — завантаження тепер називає, які саме канонічні дефолти цей файл
  перевизначає, і дає оператору шлях перенесення.

  **Проблема.** `BRAVO.config` виключений із копіювання при оновленні
  (правильно: це файл сервера). Наслідок — сервер із конфігом епохи 5.2.0
  роками не отримує жодного покращення дефолтів, і **ніде цього не
  видно**. Сам issue #154 називає це головним ризиком відмови від
  `BRAVO.config`.

  **Що зроблено.** Завантаження рахує реальну різницю між значеннями
  цього файлу й канонічними дефолтами — тим самим канонічним
  `Compare-BRAVOConfigurationGraph` (B0) і тим самим ЄДИНИМ порівнянням,
  що вже живить діагностику невідомих вкладених ключів (A3): різниця між
  ними лише в `Kind`, другого проходу графа не додано. Результат іде в
  попередження (перші 8 шляхів + кількість решти) і в метадані
  `BravoConfigurationMetadata.PrimaryConfigOverridesCanonicalDefaults`.

  **Поведінка НЕ змінена.** Файл і далі читається як primary-шар. Забрати
  його зараз означало б забрати в серверів їхні налаштування: site-значення
  парку сьогодні живуть саме там, а міграція парку (B5) ще не виконана.
  Комплект не видаляє й ніколи не видалить `BRAVO.config` сам.

  **Шлях перенесення** задокументований у `deploy/README.md`:
  `Get-BRAVOConfigSiteDelta.ps1` → `BRAVO.local.config` →
  `BRAVO_SETUP.ps1 -ValidateOnly` → лише після цього прибрати
  `BRAVO.config`.

  Покриття: `ConfigV2/StaleLegacyConfigCannotSilentlyOverrideDefaults`
  (наскрізно: застарілий конфіг називає точний шлях і при цьому НЕ ламає
  завантаження) плюс розширена
  `PrimaryStrictness/PristineConfigProducesNoDiagnostics`, яка тепер
  доводить ще й те, що комплектний `BRAVO.config` не перевизначає нічого —
  інакше попередження спрацьовувало б на кожному свіжому інсталі й
  знецінилось би.

  **Частина 2 (фізичне видалення `BRAVO.config` з пакета) НЕ зроблена**
  свідомо: кореневий файл є джерелом фікстур для 13 місць у 5 suite-ах
  self-test і для `ci\Test-BRAVOConfigFoundationParity.ps1`. Видалення
  означає переписати фікстурну тканину одночасно зі зміною
  runtime-контракту й одночасно зламати той самий parity-harness, який
  `RELEASE_POLICY` §14.4 вимагає як доказ саме для таких змін.

- **Розкатка більше не приймає prerelease мовчки (#152)** — гейт релізу
  в `deploy\Install-BRAVOServer.ps1` і `deploy\Update-BRAVOServer.ps1`
  став fail-closed.

  **Корінь.** `Install-BRAVOServer.ps1` лише друкував `[УВАГА]`, коли
  `releaseChannel != stable`, а `Update-BRAVOServer.ps1` не перевіряв
  канал **узагалі** — лише виводив його. Прапорця свідомого рішення не
  було в жодному, тобто був відсутній не лише блок, а й сам механізм
  рішення. Наслідок задокументований: production-сервер парку тривало працював у
  production на `5.2.0-rc.2`, для якого в репозиторії немає тега.

  **Провенанс перевіряється, а не друкується.** `sourceCommit` і
  `buildId` виводились, але ні з чим не звірялись: `VERSION.json`
  усередині архіву підтверджував лише той архів, у якому лежав. Тепер
  розкатка завантажує `release-manifest.json` — **окремий** ассет
  релізу — і звіряє з ним `packageVersion`, `releaseChannel`,
  `sourceCommit`, `buildId`, тег і SHA-256 архіву. Без маніфесту
  (локальний `-ZipPath`) лишаються інваріанти, які накладає сам збирач
  артефакту: `sourceCommit` — повний 40-символьний git-hash, `buildId` —
  його перші 7 символів. Відсутність маніфесту — попередження, не
  відмова: інакше зламався б документований сценарій оновлення з
  локального архіву на сервері без доступу до GitHub.

  **Одна реалізація політики, а не дві.** Рішення «що можна
  розгортати» однакове для інсталяції й оновлення, тому живе в
  `deploy\BRAVO.Deploy.ReleaseGate.ps1`; скрипти лишаються
  оркестраторами. Функції повертають рішення об'єктом, а друкує його
  викликач своїм writer-ом — інакше гейт або залежав би від функцій
  викликача неявним контрактом, або завів би третю копію писача. Ціна:
  скрипт розкатки більше не самодостатній файл, і відсутність файлу
  політики зупиняє розкатку явно (fail-closed), а не знімає гейт.

  **Найпідступніша частина — UAC.** Обидва скрипти перезапускають себе
  з піднятими правами, передаючи параметри списком. `-AllowPrereleaseChannel`
  передається далі; без цього явне рішення оператора зникало б при
  підйомі прав, гейт спрацьовував би вже під UAC, і причина виглядала б
  як дефект комплекту.

  Покриття: 17 перевірок `Deploy/*` — 6 структурних (одна реалізація
  політики, відсутність другої копії порівняння зі `stable`, передача
  прапорця через UAC, звірка з `release-manifest.json`, відповідність
  форми маніфесту збирачу артефакту) і 11 поведінкових, серед них
  `ReleaseGatePrereleaseIsRefusedByDefault`,
  `ReleaseGateMissingChannelFailsClosed` (відсутній канал — не доказ
  стабільності), `ProvenanceRejectsArtifactHashMismatch`,
  `ProvenanceRejectsTagMismatch`, `ProvenanceRejectsTruncatedSourceCommit`
  і `ProvenanceWithoutManifestWarnsButDoesNotBlock`.

  `RELEASE_POLICY.md` §16.3 фіксує політику й розводить два поняття,
  які доти змішувались: метадані гілки розробки (`sourceCommit` на
  `developer` може відставати від HEAD — це навмисно) і провенанс
  артефакту (мусить бути прив'язаний до точного ref).

  Інвентаризація парку (яка версія і `buildId` на кожному сервері) —
  операційний крок, лишається за #152.
...
- **Версійний контракт site-конфігурації (#154, B3)** —
  ...
  явний диспетч за нею.

  **Чому маркер не є перевизначенням.** Site-файл — плоский словник
  `'dot.path' = значення`, тому `configSchemaVersion` синтаксично є
  ОДНОСЕГМЕНТНИМ шляхом, тобто top-level ключем — і
  `ConvertTo-BRAVONestedOverride` відхилив би його fail-closed як
  невідомий. Маркер знімається в єдиного канонічного читача
  (`Read-BRAVOLocalConfigurationOverrides`) ДО перевірки шляхів, тож
  жоден із його п'яти споживачів його не бачить і він ніколи не
  потрапляє в ефективну конфігурацію.

  **Матриця (обсяг B3):**

  | оголошено | результат |
  | --- | --- |
  | маркера немає | версія 1, приймається + попередження + метадані |
  | `= 1` або `= 2` | приймається |
  | інше число | FAIL CLOSED |
  | не ціле число (`'2'`, `$true`, масив, `$null`) | FAIL CLOSED |

  **Рішення щодо файлів без маркера.** Кожен розгорнутий сьогодні
  site-файл маркера не має. Відмова зупинила б увесь парк на першому ж
  оновленні, тому такий файл приймається як версія 1 — але не мовчки:
  завантаження виписує попередження, а метадані несуть
  `LocalConfigDeclaredSchemaVersion` і `LocalConfigEffectiveSchemaVersion`.
  Вікно закривається само собою: обидва серіалізатори site-файлу
  (production-запис і кандидат для ізольованого effective) тепер пишуть
  маркер із однієї канонічної функції, тож файл отримує версію при
  першому ж записі через Configurator, без ручного редагування на
  сервері.

  **Версія читається з ДАНИХ, не з виконання.** `configSchemaVersion =
  1 + 1` не стає двійкою, а відхиляється AST-парсером як вираз —
  інакше маркер версії був би єдиним полем файлу, здатним обчислювати,
  тобто діркою в data-only контракті B1.

  `VERSION.json.configSchemaVersion` **не змінено**: там версія схеми
  самого комплекту, і зміна метаданих релізу разом зі зміною
  runtime-поведінки суперечить політиці релізу. Метадані завантаження
  тепер розводять ці два поняття явно.

  Покриття: 8 перевірок `ConfigVersion/*` плюс дві наскрізні
  `ConfigLoader/LocalOverrideDeclaredSchemaVersionLoads` і
  `ConfigLoader/LocalOverrideUnsupportedSchemaVersionFailsClosed`.
...
- **Health-поріг для тому, меншого за сам поріг (#155)** — коли
  ...
  фізично існує, неможливо (реальний випадок парку — том `G:` на production-сервері парку,
...
на production-сервері парку. Зведені докази —
...
операції. Наслідок відтворений на production-сервері парку 13.09.2026: 715.46 GB
...
на production-сервері парку, де раніше провалився rc.3. Усі
...
Кандидат після **FAIL acceptance rc.3** на production-сервері парку
...
### Acceptance rc.3 — FAIL (13.09.2026, production-сервер парку)
...
повним self-test і DataRestore matrix), self-test на production-сервері парку —
...
підтверджений на реальному сервері production-сервер парку (real-server acceptance
...
Real-server acceptance для rc.3 на production-сервері парку — окремий крок, продовжується
...
- **FIX (self-test, DiskSpace):** `Resolve-BRAVODiskSpaceStorageIdentity`
  ...
  self-test — на production-сервері парку (лише `C:`) усі 11 падали з
...
11 сценаріїв, що впали на production-сервері парку, тепер PASS). Real-server acceptance
для rc.2 на production-сервері парку — окремий крок, продовжується з місця зупинки rc.1.
