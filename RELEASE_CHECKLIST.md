...
## 1.1. Acceptance для релізів зі змінами кодування/сумісності архівів (A2)

Обов'язково, коли реліз змінює спосіб передачі пароля 7-Zip, кодування
stdin, чи будь-яку іншу поведінку, що впливає на читання **вже
існуючих** production-архівів (приклад: BOM-fallback для legacy
паролів, 5.2.0-rc.1) — не лише для новостворених:

- [ ] На DEV-LIMS або іншому контрольованому сервері перевірено
      generation, реально створену **попередньою** stable-версією
      (не свіжостворену цим релізом): `BRAVO_DATA_RESTORE.ps1
      -ListGenerations`, потім OutOfPlace restore цього generation,
      потім `BRAVO_RESTORE_TEST.ps1`.
- [ ] Перевірка виконана окремо під task account/`SYSTEM` **і** в
      інтерактивній операторській PowerShell-сесії — не припускається,
      що обидва середовища використовують той самий code page.
- [ ] Для обох запусків записано `[Console]::InputEncoding.CodePage` і
      `[Console]::OutputEncoding.CodePage` (docstring/evidence-файл
      acceptance), а не лише підсумковий PASS/FAIL.
