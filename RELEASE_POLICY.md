...
`BRAVO_SELF_TEST.ps1` (той набір мусить лишатись git-незалежним) і не
запускається в CI.

Тому для будь-якого PR, що змінює `BRAVO_CONFIG_LOADER.ps1`,
`modules/BRAVO.Configuration/`, `BRAVO.config` або схему
`BRAVO.local.config`, запуск harness обов'язковий, а його результат
(paritет або повний перелік відмінностей із обґрунтуванням кожної)
наводиться в описі PR. Запуск потребує Windows PowerShell 5.1 і повного
(не shallow) клону.
