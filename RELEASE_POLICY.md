...
developer: 5.2.0-rc.11 (prerelease; acceptance ще не пройдено —
...
- `v5.2.0-rc.11` (rc.10 + PR #94: компактні Maintenance-алерти —
  ...
  транспортах) — **фінальний кандидат циклу; acceptance ще не
  пройдено**; накопичена нова поверхня rc.9-rc.11 вимагає повного
  maintenance-циклу на реальному сервері (WinSCP MoveFile-міграція,
  автостворення `logs/*`, скан реальних `*.out`-варіантів, компактні
  алерти, A2-encoding протокол з RELEASE_CHECKLIST §1.1).
