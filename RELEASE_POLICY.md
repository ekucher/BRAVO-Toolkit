...
**Провенанс артефакту звіряється за вмістом, а не лише за версією.**
...
`VERSION.json` і `RUNTIME_MANIFEST.json`. Це прямий наслідок процедури
штампування: коміт-штамп за побудовою більше нічого не змінює.

Підстава: форма `sourceCommit` і рівність `packageVersion` нічого не
...
попереднього штампу тієї самої версії, проходив усі наявні перевірки,
хоч уже не описував спакований код.

**Ця звірка живе на шляху артефакту, а не в гейті PR** — і це не
недогляд. Інваріант виконується лише **на комітах-штампах**: `developer`
рухається, і після кожного мержу різниця проти `sourceCommit` законно
стає сотнями файлів. Прогін по всіх станах `VERSION.json` в історії дає
73 таких проміжних стани, кожен закритий наступним штампом. У per-PR
гейті ця перевірка була б непрохідною; на шляху тегу — точною.

Наслідок: задача збирання артефакту вимагає `fetch-depth: 0`.
`sourceCommit` — це **батько** коміта-штампа, а тег вішається на штамп,
тому при shallow-checkout потрібного коміта в репозиторії немає.
...
на **кожному** pull request. Доступний і вручну (`workflow_dispatch`).
...
#### Чому workflow запускається на КОЖЕН PR, а не за `paths`

Попередня редакція workflow фільтрувала запуск за `pull_request.paths`.
Для необов'язкової перевірки це коректно. Для **required status check**
це дефект, і саме тому фільтр прибрано:

- `paths` діє на рівні **workflow**, а не задачі. Для PR поза переліком
  workflow не запускається взагалі — немає ні прогону, ні check run, ні
  статусу;
- branch protection зіставляє required checks за **іменем** і чекає на
  статус, якого ніхто не створить. PR, що змінює лише `README.md`,
  лишився б заблокованим назавжди;
- це той самий клас інциденту, що й колізія імен під час промоції 5.1.0
  (§13.3): required check, прив'язаний до імені, якого немає.

Тому **не можна** просто ввімкнути як required check workflow із
workflow-level `paths` — ні цей, ні будь-який інший. Формулювання
«достатньо додати галочку в налаштуваннях» справедливе лише для
workflow, що запускається безумовно.

Чинна конструкція:

- workflow тригериться на `pull_request` без умов;
- рішення «релевантно / не релевантно» ухвалює **сама задача**, після
  `actions/checkout` з `fetch-depth: 0`, за змінами між точними
  `base`/`head` SHA з payload події (через `merge-base`, а не сирий
  `base.sha` — інакше результат залежав би від того, КОЛИ прогін
  стався);
- канонічний перелік шляхів конфігураційного пайплайна живе в коді —
  `ci\Test-BRAVOConfigParityRelevantPath.ps1`
  (`Get-BRAVOConfigParityRelevantPathPattern`), а не в YAML, щоб його
  можна було покрити регресією на синтетичних наборах. Станом на
  2026-09-19 він покриває `BRAVO_CONFIG_LOADER.ps1`, `BRAVO.config`,
  `BRAVO.local.config.example`, `modules/BRAVO.Configuration/**`,
  `modules/BRAVO.Configurator/**`,
  `ci\Test-BRAVOConfigFoundationParity.ps1`,
  `ci\Test-BRAVOConfigParityRelevantPath.ps1` і сам
  `.github/workflows/config-parity.yml`. Цей абзац — довідка;
  авторитетним лишається код, а звуження переліку валить self-test
  `ConfigParity/CanonicalPatternCoversRequiredPaths`;
- PR **поза** пайплайном: задача реально виконується, друкує причину й
  завершується `0` як `NOT APPLICABLE`;
- PR **у межах** пайплайна: задача запускає справжній harness без
  жодного послаблення allowlist;
- неможливість **довести** нерелевантність (відсутні SHA, недосяжний
  коміт, збій `git`) трактується як «запускати»: недоведене твердження
  не має права вимикати обов'язкову перевірку;
- ім'я `pull_request`-прогону лишається незмінним —
  `Config parity (BRAVO_CONFIG_LOADER)`; `workflow_dispatch` отримує
  суфікс ` (push)` за тією ж схемою, що й `ci.yml`, щоб ручний прогін
  над тим самим head SHA не конкурував із мерж-гейтом.

Інваріант тримається механічно: self-test
`Governance/ConfigParityWorkflowHasNoWorkflowLevelPathFilter` валить
прогін, якщо `paths`/`paths-ignore` повернуться на рівень workflow, а
...
Підстава не гіпотетична: production-сервер парку тривало працював
