...
#### Хто володіє кожним полем `VERSION.json`

`VERSION.json` — єдине джерело істини (§5.3), але поля мають **різних**
власників і оновлюються на **різних** кроках. Плутанина між ними вже
коштувала двох дефектів, тому модель зафіксована таблицею.

| Поле | Хто записує | На якому кроці | Перевіряє |
| --- | --- | --- | --- |
| `packageVersion` | людина | відкриття версії (bump-коміт) | `ci\Test-BRAVOReleasePolicy.ps1` (форма, відповідність гілці) |
| `releaseChannel` | людина | відкриття версії | `ci\Test-BRAVOReleasePolicy.ps1` (гілка, суфікс версії, перехресно `.git/HEAD`) |
| `releaseDate` | людина | відкриття версії | `ci\Test-BRAVOReleasePolicy.ps1` (формат і збіг із заголовком `CHANGELOG.md`) |
| `buildId` | `ci\Update-BRAVOVersionStamp.ps1` | штампування | self-test `Version/StampConsistency`, `ci\New-BRAVOReleaseArtifact.ps1`, `deploy\BRAVO.Deploy.ReleaseGate.ps1` |
| `sourceCommit` | `ci\Update-BRAVOVersionStamp.ps1` | штампування | ті самі три + `ci\Test-BRAVOReleasePolicy.ps1` (провенанс несе ту саму `packageVersion`) |

**`releaseDate` належить версії, а не штампу.** Це не думка, а поведінка
репозиторію: `5.1.0-dev.1` штампували 25 разів, дати комітів `sourceCommit`
охоплюють 2026-08-11…08-14, а `releaseDate` весь час лишалась `2026-08-12`.
Те саме на `5.0.0-rc.1` (6 штампів), `5.2.0-rc.2` (10) і `5.3.0-dev.2` (3).
Тому штампування дату **не чіпає**, і робити його автором цього поля було б
регресією: перештампування не створює нового релізу.

Звідси й корінь дрейфу: поле оновлює саме bump-коміт, а в `5.3.0-dev.2` і
далі в `5.3.0-dev.3` версію піднімали без цього кроку (dev.3 — узагалі
всередині merge-резолюції), і дата rc.1 поїхала через усю лінію.

**`sourceCommit` — це коміт коду релізу, а не коміт-штамп.** Самопосилання
неможливе: у коміті-штампі `sourceCommit` вказує на його батька, бо власний
hash відомий лише після створення коміта. Саме тому розгортати треба **тег**
(коміт-штамп), а не коміт коду — процедура детально описана в
`ci\Update-BRAVOVersionStamp.ps1`. `ci\New-BRAVOReleaseArtifact.ps1` пише в
`release-manifest.json` **обидва**: `sourceCommit` із пакета і фактичний
`archiveCommit` спакованого ref-а; вони навмисно різні й не звіряються між
собою.

Наслідок для PR, що штампує: у squash-merge комітів гілки не лишається, тому
`sourceCommit` вказують на коміт `developer`, на якому гілку засновано. Він
і стає батьком squash-коміта — тобто модель зберігається.

**Інваріант, обов'язковий для будь-якої merge-стратегії:** `sourceCommit`
має лишатися reachable з цільової релізної гілки й ідентифікувати саме той
код, що описаний стемпованими метаданими. Якщо коміт-штамп PR посилається
на інший коміт коду **всередині того самого PR** (а не на вже наявний
коміт `developer`, на якому PR засновано), такий PR **не можна
squash-merge**: squash знищує ідентичність коміту, на який вказує
`sourceCommit`. У цьому випадку — лише merge-коміт.

**Чому три копії правила `buildId == short(sourceCommit)` лишаються.**
`deploy\BRAVO.Deploy.ReleaseGate.ps1` виконується на сервері установи, де
немає ані репозиторію, ані `ci\`; self-test мусить лишатись git-незалежним
(§14.4); збирач артефакту працює до того, як пакет узагалі існує. Спільної
точки, доступної всім трьом, немає — тож це свідома дублікація одного
інваріанта на трьох межах довіри, а не розмноження політики.
...
### Де дивитись поточний стан

Цей розділ **не є** дашбордом поточного стану і більше ним не буде.
Попередня редакція тримала рядки на кшталт `master: 5.2.0` і
`developer: 5.3.0-dev.2` вручну — і відстала на чотири stable-релізи
(5.2.1, 5.2.2, 5.2.3, 5.2.4), бо жоден механізм її не звіряв. Дані, які
застарівають щоразу при штампуванні версії, не належать у нормативний
документ: §9.3 робить відповідність документації фактичній поведінці
критерієм готовності RC, а недостовірний розділ цей критерій лише
імітує.

Авторитетні джерела поточного стану — машинні:

| Питання | Джерело істини |
| --- | --- |
| Яка версія і канал у гілці | `VERSION.json` у самій гілці (§5.3) |
| Який провенанс у версії | `VERSION.json.sourceCommit`/`buildId` (§7.2) |
| Які релізи існують | git-теги `v*` і GitHub Releases (§15, §16) |
| Чи узгоджені гілка, версія, канал, дата й провенанс | `ci\Test-BRAVOReleasePolicy.ps1` (§13.3, required check) |

Нижче лишається **хронологія прийнятих релізів** — це докази acceptance,
а не стан. Вони не застарівають: минулий реліз лишається тим, чим був.

### Незакритий кандидат циклу 5.3.0

`v5.3.0-rc.1` (stamp `3a87079`, sourceCommit `f4351a5`, 2026-08-26) —
**immutable кандидат, який приймання не проходив**. Цикл 5.3.0 свідомо
повернуто в development за рішенням власника, щоб внести P0
Configuration Foundation: §3.2 забороняє нові функції в RC. Тег
лишається як є (§15.3), GitHub Release для нього лишається draft-ом,
наступний кандидат циклу — `rc.2`.
