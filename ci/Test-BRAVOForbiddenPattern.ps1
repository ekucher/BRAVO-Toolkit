...
$forbiddenRules = @(
    ...
    @{
        # Прискорювачі типу, яких немає у Windows PowerShell 5.1 (додані
        # лише в PowerShell 6+). Реальний інцидент CI 2026-09-15:
        # обчислення такого операнда кидає "Unable to find type" і валить
        # усе завантаження конфігурації. У ланцюзі -or дефект невидимий
        # доти, доки якийсь ранній операнд істинний і ланцюг замикається
        # достроково, тому PSScriptAnalyzer і звичайні тести його
        # пропускають — ловить лише текстове сканування.
        # Перелік навмисно містить рівно ту пару, чию відсутність у 5.1
        # підтверджено прогоном, а не здогадку про решту прискорювачів.
        # Повні імена типів ([System.Int16]) лишаються дозволеними.
        Name = 'Прискорювач типу, відсутній у Windows PowerShell 5.1'
        Pattern = '(?i)\[(short|ushort)\]'
        Allowlist = @()
    },
    @{
