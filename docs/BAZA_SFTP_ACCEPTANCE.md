...
append-only двигун (`BRAVO.BazaSync`, комплект зі stamp `4845d4d`,
code `44286ae`) поводиться в 10 критичних сценаріях так само, як у
синтетичному self-test (964 PASS). Прогін виконується **до** merge
`developer → master`; за результатами ухвалюється рішення про реліз.
...
1. **Розгорнутий комплект** відповідає stamp `4845d4d`
   (`BRAVO_RUNTIME_GUARD.ps1` проходить; `VERSION.json`:
   `sourceCommit=44286ae…`, `buildId=44286ae`).
