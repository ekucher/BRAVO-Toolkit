...
Статус: **PASS з відкритими позиціями** — сценарій 1 і ядро §9.1 закриті на
реальному сервері; чотири пункти §9.1 не виконувались (розділ 8). Документ
...
| Restore test з explicit `-GenerationId` | **не виконувався** | розділ 8 |
...
Restore test result:  не виконувався
...
1. **Запуск від `NT AUTHORITY\SYSTEM`** через Task Scheduler (усі прогони —
   MANUAL під `LIMS-TOP\LimsTop`). §9.1 називає це обов'язковим.
2. **`BRAVO_DRY_RUN.ps1 -TestAccess` під SYSTEM** — виконувався READ-ONLY під
   інтерактивним обліковим записом.
3. **Restore test з explicit `-GenerationId`** — не виконувався. Синтетичне
   покриття є: `BRAVO_DATA_RESTORE_MATRIX_TEST.ps1` зелений у CI.
