...
**Історична примітка (#154, крок 2):** цю ручну процедуру автоматизує
`docs/BRAVO_CONFIG_V2_PILOT_RUNBOOK.md` + `deploy\Start-BRAVOConfigV2Pilot.ps1`
(pilot-артефакт). Цей документ лишається як історичний доказ acceptance
(PR #207) і як ручний fallback, якщо автоматизований інструмент
використати не можна. Обидва описують ту саму послідовність доказів.
